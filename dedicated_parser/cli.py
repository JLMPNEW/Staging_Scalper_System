...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the shared SEC parser against existing database/cache content."
    )
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Rebuild assessments and funnel artifacts from an existing run "
            "without reparsing source documents."
        ),
    )
...
def _ticker_list(raw: object) -> list[str]:
    return sorted(
        {
            ticker.strip().upper()
            for ticker in str(raw or "").split(",")
            if ticker.strip()
        }
    )
...
def _accession_list(raw: object) -> list[str]:
    return sorted(
        {
            accession.strip()
            for accession in str(raw or "").split(",")
            if accession.strip()
        }
    )
...
def _export_policy_corpus(registry: AdapterRegistry) -> None:
    ...
    if not registry.review_policy_path or not registry.review_policy_golden_path:
        raise ValueError(
            "Adapter registry must configure both review_policy_path and "
            "review_policy_golden_path"
        )
...
def reassess_existing_run(
    ...
    *,
    ...
    if str(run["model_family"]) != registry.model_family:
        raise ValueError(
            f"run_id={run_id} belongs to {run['model_family']!r}, "
            f"not {registry.model_family!r}"
        )
    asof_date = str(run["asof_date"])
    if requested_asof and requested_asof != asof_date:
        raise ValueError(
            f"--asof {requested_asof} does not match run_id={run_id} "
...
def _write_run_artifacts(
    *,
    ...
    assessment_path = output_json.with_name(
    ...
    write_assessment_csv(
        ...
        [
            ...
            if row["recovery_class"] in REVIEW_CLASSES
        ],
    )
    funnel_path = output_json.with_name(
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.reassess_run_id:
        ...
        if invalid:
            raise ValueError(
                "--reassess-run-id cannot be combined with "
                + ", ".join(invalid)
            )
    elif not args.asof:
    ...
    if not args.reassess_run_id and args.cache_dir is None:
        raise ValueError(
            "--cache-dir is required unless --reassess-run-id is used"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    with connect_database(args.db) as conn:
        ...
        work, summary = build_plan(
            ...
            pdf_extraction_timeout_seconds=(
                args.pdf_extraction_timeout_seconds
            ),
        )
        if args.plan_only:
            payload = {
    ...
    payload = {
        ...
        "comparison_status_counts": {
            status: sum(
                row["comparison_status"] == status for row in comparison
            )
            for status in sorted(
                {str(row["comparison_status"]) for row in comparison}
            )
        },
        "recovery_assessment": assessment_summary(assessments),
        "extraction_funnel": {
