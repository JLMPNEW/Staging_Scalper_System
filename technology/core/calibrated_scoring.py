...
class CalibratedScoringSettings:
    ...
    default_baseline_source_id: str
    default_model_version: str
    build_run_type: str
    validation_run_type: str
    default_component_weights: dict[str, float]
    default_overlay_components: list[str]
...
def parse_args(description: str, default_config: Path) -> argparse.Namespace:
...
def configured_overlay_names(config: dict[str, Any], settings: CalibratedScoringSettings) -> list[str]:
    raw = cfg_get(config, f"{settings.config_key}.sector_overlay_components", None)
    if raw is None:
        return list(settings.default_overlay_components)
    return list(raw or [])


def resolve_model_family(config: dict[str, Any], settings: CalibratedScoringSettings) -> str:
    return str(
        cfg_get(
            config,
...
def subfeature_weight_specs(config: dict[str, Any], settings: CalibratedScoringSettings) -> dict[str, list[tuple[str, float]]]:
    raw = cfg_get(config, f"{settings.config_key}.subfeature_weights", {})
    ...
    for component, raw_weights in raw.items():
        if isinstance(raw_weights, dict):
            ...
            if unknown_keys:
                raise ValueError(f"Unknown subfeature score keys in {settings.config_key}.subfeature_weights.{component}: {unknown_keys}")
...
def recalibrate_components(
    ...
    *,
    ...
    component_names = [component["component_name"] for component in CORE_COMPONENT_DEFS] + overlay_names
    for row in rows:
        row["_component_meta"] = {}
...
def build_calibrated_scores(settings: CalibratedScoringSettings) -> None:
    configure_utc_logging()
    args = parse_args(settings.build_description, settings.default_config)
    ...
    with connect(db_path, timeout_sec=float(cfg_get(config, "runtime.sqlite_timeout_sec", 30.0))) as conn:
        ...
        run_id = start_run(conn, run_type=settings.build_run_type, input_path=config_path)
        try:
            ...
            with conn:
                ...
                for output in outputs:
                    ...
                    if str(output["model_status"]) != "complete":
                        add_issue(conn, row, source_id=source_id, stage=settings.build_run_type, detail=str(output["review_reason"] or "review"))
...
def validate_calibrated_scores(settings: CalibratedScoringSettings) -> int:
    configure_utc_logging()
    args = parse_args(settings.validate_description, settings.default_config)
    config_path = args.config.expanduser().resolve()
    config = load_yaml(config_path)
    base_dir = config_path.parent
    db_path = args.db.expanduser().resolve() if args.db else resolve_path(cfg_get(config, "paths.database_path"), base_dir=base_dir)
    ...
    with connect(db_path, timeout_sec=float(cfg_get(config, "runtime.sqlite_timeout_sec", 30.0))) as conn:
        ...
        if require_zero_growth and growth_weight > 0.001:
...
if __name__ == "__main__":
    raise SystemExit("Use a subsector wrapper script for calibrated scoring.")
