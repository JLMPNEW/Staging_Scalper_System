...
from technology.core.text_norm import as_bool, normalize_cik, normalize_label, normalize_org_name, normalize_ticker  # noqa: E402


LOGGER = logging.getLogger("load_technology_universe")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load a technology subsector universe into technology.sqlite.")
    ...
    for idx, raw in enumerate(cohorts, start=1):
        ...
        for raw_ticker in tickers:
            ...
            if not ticker:
                continue
            if ticker in out:
                raise ValueError(f"Ticker {ticker} appears in multiple semiconductor cohorts.")
...
def data_quality_status(company: UniverseCompany) -> str:
    ...
    if not company.cik:
        return "missing_cik"
    if company.calibration_cohort_id == "semi_unassigned":
...
def parse_universe_rows(
    ...
    *,
    ...
    unassigned_id = str(policy.get("default_unassigned_cohort_id") or "semi_unassigned").strip()
    unassigned_name = str(policy.get("default_unassigned_cohort_name") or "Unassigned semiconductor review").strip()
    ...
    for raw in rows:
        ...
        company = UniverseCompany(**{**company.__dict__, "data_quality_status": data_quality_status(company)})
    ...
    if expected_ticker_count > 0 and len(companies) != expected_ticker_count:
        raise ValueError(
            f"{path} is the semiconductor source of truth and must contain exactly "
            f"{expected_ticker_count} unique tickers; found {len(companies)}."
        )
    return companies


def clear_stage_issues(conn: Any, tickers: list[str]) -> None:
    if not tickers:
        return
...
def upsert_universe(conn: Any, companies: list[UniverseCompany]) -> int:
    now = utc_now()
    seed_source_id = source_id_or_none(conn, SEED_SOURCE_ID)
    cohort_source_id = source_id_or_none(conn, COHORT_SOURCE_ID)
    sec_source_id = source_id_or_none(conn, "sec_company_tickers")
    clear_stage_issues(conn, [company.ticker for company in companies])
    ...
    for company in companies:
        ...
        if not company.cik:
            add_issue(
                ...
                issue_detail="CIK is missing from semiconductor_tickers.csv; permitted for some foreign OTC/ADR names but excluded from SEC-fundamental coverage until resolved.",
                severity="warning",
            )
        if company.calibration_cohort_id == "semi_unassigned":
            add_issue(
                ...
                issue_type="unassigned_semiconductor_cohort",
                issue_detail="Ticker is not assigned to one of the four core semiconductor calibration cohorts.",
                severity="warning",
            )
...
def main() -> None:
    ...
    companies = parse_universe_rows(universe_csv, policy=policy, cohort_map=cohort_map, model_family=model_family)
    ...
    with connect(db_path, timeout_sec=timeout_sec) as conn:
        ...
        run_id = start_run(conn, run_type=LOAD_STAGE, input_path=universe_csv)
        try:
            with conn:
                row_count = upsert_universe(conn, companies)
            missing_cik = sum(1 for company in companies if not company.cik)
            unassigned = sum(1 for company in companies if company.calibration_cohort_id == "semi_unassigned")
            finish_run(
                conn,
                run_id=run_id,
                status="success",
