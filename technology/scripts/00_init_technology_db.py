...
from technology.core.db import connect, finish_run, init_db, start_run, table_names  # noqa: E402
...
LOGGER = logging.getLogger("init_technology_db")
DEFAULT_CONFIG = PACKAGE_ROOT / "config.yaml"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Initialize the independent technology SQLite database.")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--db", type=Path, default=None, help="Override SQLite database path.")
    parser.add_argument("--source-registry", type=Path, default=None, help="Override source registry YAML.")
    parser.add_argument("--skip-source-registry", action="store_true", help="Create schema without loading sources.")
...
def main() -> None:
    ...
    with connect(db_path, timeout_sec=timeout_sec) as conn:
        init_db(conn)
        run_id = start_run(conn, run_type="init_technology_db", input_path=config_path)
        try:
            source_count = 0
            ...
            LOGGER.info("Initialized technology DB: %s", db_path)
