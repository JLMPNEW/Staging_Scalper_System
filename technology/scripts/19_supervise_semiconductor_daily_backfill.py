#!/usr/bin/env python3
"""Supervise semiconductor daily dashboard backfill in restartable chunks."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Supervise semiconductor daily dashboard backfill in chunks.")
...
def run_chunk(
    *,
    ...
    child_log = log_path.parent / f"semiconductors_daily_chunk_{chunk_id}_{utc_stamp()}.log"
    cmd = [
        sys.executable,
        str(RUNNER),
        "--db",
...
def main() -> int:
    ...
    log_path = log_dir / f"semiconductors_daily_supervisor_{args.start_date}_to_latest_{utc_stamp()}.log"
    status_path = (
        args.status_json.expanduser().resolve()
        if args.status_json
        else log_dir / f"semiconductors_daily_supervisor_{args.start_date}_status.json"
    )
    ...
    log_line(log_path, f"SUPERVISOR start dates={len(dates)} chunks={len(date_chunks)} range={args.start_date}..{status['end_date']}")
    ...
    write_status(status_path, status)
    log_line(log_path, f"SUPERVISOR complete through={dates[-1]}")
    return 0
