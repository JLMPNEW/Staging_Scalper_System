...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit existing software specialized-metric coverage without changing the database."
    )
...
def main() -> None:
    ...
    applicability = load_applicability_rows(
    ...
    with open_read_only_database(db_path, timeout_sec=timeout_sec) as conn:
        ...
        metric_summary, metric_detail = build_metric_gap_census(
            conn,
            registry=registry,
            members=members,
            asof_date=asof_date,
        )
        applicability_census = build_applicability_census(
            applicability_rows=applicability,
            members=members,
            asof_date=asof_date,
        )
        filing_census = build_filing_census(
            conn,
            members=members,
            registry=registry,
            history_start_date=history_start_date,
    ...
    manifest = write_baseline_outputs(
        ...
        applicability_census=applicability_census,
        filing_census=filing_census,
    )
