...
TICKERS_CSV = PROJECT_ROOT / "ticker_mapping" / "software_infrastructure_tickers.csv"
HISTORICAL_CSV = PACKAGE_ROOT / "software_infrastructure" / "data" / "software_infrastructure_historical_membership.csv"
POSITIONING_UNIVERSE_CSV = (
    PROJECT_ROOT / "output" / "technology_cache" / "positioning" / "software_infrastructure_positioning_universe.csv"
)
POSITIONING_UNIVERSE_AUDIT_CSV = (
    PROJECT_ROOT / "output" / "technology_cache" / "positioning" / "software_infrastructure_positioning_universe_skipped.csv"
)
...
def build_positioning_universe() -> Path:
    current_rows = read_rows(TICKERS_CSV)
    ...
    for row in historical_rows:
        internal_ticker = str(row.get("internal_ticker") or "").strip().upper()
        ...
        rows_by_ticker[exchange_ticker] = {
            ...
            "subsector": str(row.get("calibration_cohort") or "Software Infrastructure"),
