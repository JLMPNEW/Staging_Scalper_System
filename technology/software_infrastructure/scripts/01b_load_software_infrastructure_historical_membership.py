#!/usr/bin/env python3
"""Load point-in-time software-infrastructure research membership rows."""
...
LOGGER = logging.getLogger("load_software_infrastructure_historical_membership")
...
SOURCE_ID = "software_infrastructure_historical_membership_seed"
RUN_TYPE = "load_software_infrastructure_historical_membership"
POSITIONING_AMBIGUOUS_EXCHANGE_TICKERS = {"INFA", "SAIL", "SWI"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load software-infrastructure historical/delisted PIT membership.")
...
def delete_existing_current_memberships(conn: Any) -> int:
    """Keep one current PIT interval per active software ticker after this loader runs."""
...
def upsert_historical_company(conn: Any, row: dict[str, str], *, source_id: str) -> int:
    ...
    cohort_id = str(row.get("calibration_cohort_id") or "softinfra_historical")
    cohort_name = str(row.get("calibration_cohort") or "Historical software infrastructure membership")
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, 'Technology', 'Software - Infrastructure', 'Software Infrastructure',
    ...
    conn.execute(
        ...
        (
            ...
            cohort_id,
            cohort_name,
            str(row.get("notes") or row.get("event_type") or "historical_software_infrastructure")[:250],
...
def main() -> int:
    ...
    with connect(db_path, timeout_sec=float(cfg_get(config, "runtime.sqlite_timeout_sec", 120.0))) as conn:
        ...
        try:
            ...
            LOGGER.info(
                "Loaded software-infrastructure PIT membership: current=%d historical=%d pruned_current=%d db=%s",
                current_count,
                historical_count,
                pruned_current,
                db_path,
