...
def main() -> int:
    ...
    if summary_rows:
        ...
        rollback_model = str(cfg_get(config, f"{CONFIG_KEY}.rollback_shadow_model_name", ""))
        ...
        if rollback_model and rollback_model not in models:
            errors.append(f"Rollback shadow missing from backtest summary: {rollback_model}")
