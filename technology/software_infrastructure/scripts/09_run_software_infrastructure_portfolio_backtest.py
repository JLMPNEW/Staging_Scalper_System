#!/usr/bin/env python3
"""Stage 9 report-only portfolio simulations for software infrastructure scores."""
...
LOGGER = logging.getLogger("software_infrastructure_portfolio_backtest")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Stage 9 software infrastructure portfolio simulations.")
...
def load_candidates(config: dict[str, Any], base_dir: Path) -> list[dict[str, Any]]:
    production_name = str(cfg_get(config, f"{CONFIG_KEY}.production_model_name", "stage8_promoted_production_v1"))
    production_type = str(cfg_get(config, f"{CONFIG_KEY}.production_model_type", "production"))
    out: list[dict[str, Any]] = [
        {
            "model_name": production_name,
            "model_type": production_type,
...
def main() -> int:
    ...
    neutral_score = float(cfg_get(config, "software_infrastructure_scoring_features.neutral_score", 50.0))
    ...
    if not specs:
        raise RuntimeError("No software portfolio specs configured.")
