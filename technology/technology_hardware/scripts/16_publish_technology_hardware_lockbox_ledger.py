#!/usr/bin/env python3
"""Publish technology-hardware lockbox ledger and signal registry reports."""
...
LOGGER = logging.getLogger("technology_hardware_governance")
DEFAULT_CONFIG = PACKAGE_ROOT / "config.yaml"
CONFIG_KEY = "technology_hardware_governance_reports"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish technology-hardware lockbox ledger and signal registry.")
...
def git_commit() -> str:
    try:
        result = subprocess.run(["git", "rev-parse", "--short", "HEAD"], cwd=PROJECT_ROOT, check=False, capture_output=True, text=True)
    except OSError:
        return ""
...
def fallback_diag(index: dict[tuple[str, str, int], dict[str, Any]], signal: str, component: str, horizon: int) -> tuple[dict[str, Any] | None, str]:
...
def stage8_candidate_map(config: dict[str, Any]) -> dict[str, set[str]]:
    raw = cfg_get(config, "technology_hardware_optuna_calibration.subfeature_candidates", {}) or {}
...
def decision_status(policy: str, production_effective_weight: float, stage8_effective_weight: float, keep_21: int, keep_63: int) -> str:
    policy = policy.strip().lower()
    if production_effective_weight > 0:
        return "production_locked"
    if policy.startswith("planned"):
        return "planned_not_loaded"
    if policy.startswith("measurement"):
    ...
    if stage8_effective_weight > 0:
        return "stage8_report_only_candidate" if keep_21 or keep_63 else "stage8_candidate_weak_diagnostics"
    return "review"


def build_signal_registry_rows(registry: dict[str, Any], config: dict[str, Any], diagnostics_rows: list[dict[str, Any]], birthdate_rows: list[dict[str, Any]], stage8_weights: dict[str, Any]) -> list[dict[str, Any]]:
            ...
            rows.append(
                {
                    ...
                    "decision_status": decision_status(policy, production_effective_weight, stage8_effective_weight, keep21, keep63),
                    ...
                    "stage8_candidate_flag": int(score_key in stage8_candidates.get(component, set())),
...
def top_ranked(conn: sqlite3.Connection, source_id: str, model_family: str, limit: int = 10) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        FROM feature_scoring_model_output
        WHERE source_id = ? AND model_family = ? AND rank_ready_flag = 1
          AND asof_date = (
              SELECT MAX(asof_date)
              FROM feature_scoring_model_output
              WHERE source_id = ? AND model_family = ?
...
def top_stage8_candidates(rows: list[dict[str, Any]], limit: int = 10) -> list[dict[str, Any]]:
    return sorted(rows, key=lambda row: safe_int(row.get("stage8_candidate_rank")) or 10**9)[:limit]
...
def performance_delta(left: dict[str, Any], right: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for field in ("annualized_return", "sharpe", "max_drawdown", "avg_excess_return_vs_qqq", "avg_excess_return_vs_equal_weight"):
        left_value = safe_float(left.get(field))
        right_value = safe_float(right.get(field))
        out[f"{field}_delta"] = right_value - left_value if left_value is not None and right_value is not None else ""
    return out
...
def main() -> int:
    ...
    registry_path = args.registry.expanduser().resolve() if args.registry else resolve_path(
        cfg_get(config, f"{CONFIG_KEY}.signal_registry_path", "technology_hardware/data/technology_hardware_signal_registry.yaml"),
        base_dir=base_dir,
    )
    ...
    diagnostics_dir = resolve_path(cfg_get(config, "technology_hardware_signal_diagnostics.output_dir"), base_dir=base_dir)
    scoring_dir = resolve_path("../output/technology_reports/technology_hardware/scoring", base_dir=base_dir)
    optuna_dir = resolve_path(cfg_get(config, "technology_hardware_optuna_calibration.output_dir"), base_dir=base_dir)
    backtest_dir = resolve_path(cfg_get(config, "technology_hardware_portfolio_backtest.output_dir"), base_dir=base_dir)
    dashboard_dir = resolve_path(cfg_get(config, "technology_hardware_dashboard_reports.output_dir"), base_dir=base_dir)
    ...
    signal_rows = build_signal_registry_rows(registry, config, subfeature_rows, birthdate_rows, stage8_weights)
        ...
        json.dumps(
            {
                ...
                "model_family": registry.get("model_family", "technology_hardware"),
                "rows": signal_rows,
            },
            indent=2,
            sort_keys=True,
    ...
    production_source_id = str(cfg_get(config, "technology_hardware_calibrated_scoring.source_id", "technology_hardware_calibrated_score_v1"))
    model_family = str(cfg_get(config, "technology_hardware_calibrated_scoring.model_family", "technology_hardware"))
    with readonly_connect(db_path) as conn:
        stage7_summary = latest_stage7_summary(conn, production_source_id, model_family)
        top10_stage7 = top_ranked(conn, production_source_id, model_family, limit=10)
    ...
    backtest_rows = read_csv_rows(backtest_dir / "technology_hardware_portfolio_backtest_summary.csv")
    dashboard_manifest = read_json(dashboard_dir / "technology_hardware_dashboard_manifest.json")
    risk_flags = read_csv_rows(dashboard_dir / "technology_hardware_risk_flags.csv")
    production_model_name = str(cfg_get(config, "technology_hardware_portfolio_backtest.production_model_name", "stage7_current_production_v1"))
    stage8_model_name = "stage8_best_candidate_report_only"
    ...
    artifacts = [
        ...
        artifact_row("stage6_scoring_contract", "model_input", scoring_dir / "technology_hardware_scoring_feature_contract.csv"),
        artifact_row("production_calibrated_scores", "production_scores", scoring_dir / "technology_hardware_stage7_calibrated_scores.csv"),
        artifact_row("production_validation", "validation", scoring_dir / "technology_hardware_stage7_validation.csv"),
        artifact_row("stage7_challenger_scores", "reference_scores", scoring_dir / "technology_hardware_stage7_challenger_scores.csv"),
        artifact_row("stage7_challenger_validation", "reference_validation", scoring_dir / "technology_hardware_stage7_challenger_validation.csv"),
        ...
        artifact_row("portfolio_backtest_summary", "backtest", backtest_dir / "technology_hardware_portfolio_backtest_summary.csv"),
        artifact_row("portfolio_backtest_manifest", "backtest", backtest_dir / "technology_hardware_portfolio_backtest_manifest.json"),
        artifact_row("dashboard_manifest", "dashboard", dashboard_dir / "technology_hardware_dashboard_manifest.json"),
        artifact_row("dashboard_final_rank_table", "dashboard", dashboard_dir / "technology_hardware_final_rank_table.csv"),
        artifact_row("dashboard_stage8_candidate_rank_table", "dashboard", dashboard_dir / "technology_hardware_stage8_candidate_rank_table.csv"),
    ...
    lockbox = {
        ...
        "snapshot_id": f"technology_hardware_lockbox_{snapshot_stamp}",
        ...
        "stage7_reference_status": cfg_get(config, f"{CONFIG_KEY}.stage7_challenger_status", "same_weights_reference"),
        "stage8_candidate_model_name": stage8_model_name,
        "stage8_candidate_status": stage8_status,
        "latest_stage8_research_candidate_status": latest_stage8_status,
        ...
        "recommended_stage8_use_case": cfg_get(config, f"{CONFIG_KEY}.recommended_stage8_use_case", "research_only"),
        "promotion_confidence": cfg_get(config, f"{CONFIG_KEY}.promotion_confidence", "not_promoted"),
        "promotion_decision": {
            "decision": "stage7_remains_production",
            "stage8_is_production": 0,
            "latest_research_candidate_promoted": 0,
            "reason": "Stage 8 hardware calibration is preserved as report-only because promotion gates did not pass and Stage 9 favored the Stage 7 production baseline.",
        },
        ...
        "stage8_research_decision": {
            ...
            "research_candidate_status": latest_stage8_status,
        ...
        "backtest_reference": {
            "stage7_production_top_quintile_score_weight_long_only": production_reference,
            "stage8_candidate_top_quintile_score_weight_long_only": stage8_reference,
            "stage8_minus_stage7": performance_delta(production_reference, stage8_reference),
        },
        ...
        "signal_registry_summary": {
            ...
            "stage8_report_only_candidate": sum(1 for row in signal_rows if row.get("decision_status") == "stage8_report_only_candidate"),
        ...
        "missing_required_artifacts": [row["artifact_name"] for row in artifacts if row["required_flag"] and not row["exists_flag"]],
        "artifacts": artifacts,
    }
    ...
    manifest_json = output_dir / "technology_hardware_governance_manifest.json"
    ...
    manifest = {
        ...
        "backtest_reference_delta": lockbox["backtest_reference"]["stage8_minus_stage7"],
    }
    manifest_json.write_text(json.dumps(manifest, indent=2, sort_keys=True, default=str), encoding="utf-8")
    LOGGER.info(
        "Published technology-hardware governance reports: missing_required=%d output=%s",
