#!/usr/bin/env python3
"""Stage 10 technology-hardware dashboard/report publisher."""
...
LOGGER = logging.getLogger("technology_hardware_dashboard_reports")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish Stage 10 technology-hardware dashboard reports.")
...
def component_rows(conn: sqlite3.Connection, *, source_id: str, model_family: str, asof: str) -> list[dict[str, Any]]:
    return fetch_dicts(
        conn,
        """
        SELECT ticker, component_name, component_group, component_score, universe_percentile,
               cohort_percentile, component_quality, component_status,
               available_subfeature_count, missing_subfeature_count, default_applied, review_reason
...
def component_summary(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    out: list[dict[str, Any]] = []
    for component, comp_rows in sorted(buckets.items()):
        scores = [value for value in (safe_float(row.get("component_score")) for row in comp_rows) if value is not None]
        qualities = [value for value in (safe_float(row.get("component_quality")) for row in comp_rows) if value is not None]
        out.append(
            {
                "component_name": component,
                "ticker_count": len(comp_rows),
                ...
                "review_count": sum(1 for row in comp_rows if str(row.get("component_status") or "").lower() != "complete"),
                "default_applied_count": sum(1 for row in comp_rows if safe_int(row.get("default_applied")) == 1),
                "total_missing_subfeatures": sum(safe_int(row.get("missing_subfeature_count")) for row in comp_rows),
            }
        )
    return out
...
def backtest_leaders(rows: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
    ...
    fields = [
        "model_name", "portfolio_name", "weight_method", "exposure_mode", "annualized_return",
        "annualized_vol", "sharpe", "max_drawdown", "avg_excess_return_vs_qqq",
        "avg_excess_return_vs_equal_weight", "avg_turnover", "avg_total_cost", "avg_max_cohort_share",
    ]
...
def calibration_summary(config: dict[str, Any], base_dir: Path) -> list[dict[str, Any]]:
    ...
    rows: list[dict[str, Any]] = [
        {"section": "stage8a_signal_diagnostics", "metric": "panel_rows", "value": stage8a.get("usable_panel_rows", stage8a.get("panel_rows", "")), "detail": f"dates={stage8a.get('panel_dates', '')}"},
        {"section": "stage8a_signal_diagnostics", "metric": "date_range", "value": f"{stage8a.get('start_date', stage8a.get('min_panel_date', ''))}..{stage8a.get('end_date', stage8a.get('max_panel_date', ''))}", "detail": ""},
        {"section": "stage8b_optuna", "metric": "promotion_candidate", "value": weights.get("promotion_candidate", ""), "detail": f"source={weights.get('source_id', '')}"},
        {"section": "stage8c_walk_forward", "metric": "procedure_adds_value", "value": walk_forward.get("procedure_adds_value", ""), "detail": f"blocks={walk_forward.get('n_blocks', '')} win_rate={walk_forward.get('refit_win_rate', '')}"},
        {"section": "stage8c_walk_forward", "metric": "mean_objective_improvement", "value": walk_forward.get("mean_objective_improvement", ""), "detail": f"gate_pass_rate={walk_forward.get('promotion_gate_pass_rate', '')}"},
    ]
        ...
        rows.append(
            {
                ...
                "detail": f"ic21={row.get('holdout_mean_ic_21', '')} ic63={row.get('holdout_mean_ic_63', '')} spread21_net={row.get('holdout_mean_spread_net_21', '')}",
            }
        )
    return rows
...
def html_table(rows: list[dict[str, Any]], columns: list[str], limit: int | None = None) -> str:
    ...
    body = "".join(
        "<tr>" + "".join(f"<td>{html.escape(fmt(row.get(col, '')))}</td>" for col in columns) + "</tr>"
        for row in subset
    )
...
<head>
  <meta charset="utf-8">
  <title>Technology Hardware Dashboard</title>
  <style>
