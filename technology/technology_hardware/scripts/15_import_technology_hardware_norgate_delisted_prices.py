#!/usr/bin/env python3
"""One-time Norgate price import for technology-hardware historical members."""
...
LOGGER = logging.getLogger("import_technology_hardware_norgate_delisted_prices")
...
RUN_TYPE = "technology_hardware_norgate_delisted_price_import"
REPORT_FIELDS = [
    "ticker",
    "exchange_ticker",
    "company_name",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Norgate delisted technology-hardware OHLCV rows.")
...
def ensure_source(conn: sqlite3.Connection, source_id: str, timestamp: str) -> None:
    conn.execute(
        ...
        (
            ...
            "fact_price_ohlcv",
            "technology_hardware_calibration,technology_hardware_backtest,survivorship_backfill",
            "technology",
            15,
            "active",
            "Historical/delisted technology-hardware price backfill. Rows preserve internal historical ticker keys and record source_symbol in price_adjustment.",
            timestamp,
            timestamp,
        ),
    )
