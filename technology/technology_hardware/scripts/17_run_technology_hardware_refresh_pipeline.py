#!/usr/bin/env python3
"""Run the validated technology-hardware refresh sequence.

The default path refreshes production inputs, rebuilds Stage 6/7 scores,
publishes dashboard/governance artifacts, and runs validators. Research
diagnostics, Optuna calibration, portfolio backtests, and one-time Norgate
backfill are opt-in so routine refreshes do not silently change the model
review surface.
"""
...
CONFIG_KEY = "technology_hardware_refresh_pipeline"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the technology-hardware production refresh pipeline.")
...
def build_steps(
    *,
    ...
    return [
        ...
        Step("01_load_universe", "stage_2", "Load current technology-hardware universe", py_script("technology/technology_hardware/scripts/01_load_technology_hardware_universe.py")),
        Step("02_validate_universe_initial", "stage_2", "Validate current technology-hardware universe", py_script("technology/technology_hardware/scripts/02_validate_technology_hardware_universe.py")),
        Step("03_sync_prices", "stage_3", "Sync Yahoo adjusted prices and benchmarks", py_script("technology/technology_hardware/scripts/03_sync_technology_hardware_prices.py"), [*asof_args, *refresh_args], network=True),
        Step("01b_load_historical_membership", "stage_2", "Load PIT historical/current technology-hardware membership after price availability", py_script("technology/technology_hardware/scripts/01b_load_technology_hardware_historical_membership.py")),
        Step("02_validate_universe", "stage_2", "Validate PIT technology-hardware universe", py_script("technology/technology_hardware/scripts/02_validate_technology_hardware_universe.py")),
        Step("15_norgate_backfill", "stage_15", "Import Norgate delisted prices", py_script("technology/technology_hardware/scripts/15_import_technology_hardware_norgate_delisted_prices.py"), norgate_backfill=True),
        Step("05_build_market_features", "stage_3", "Build market technical features", py_script("technology/technology_hardware/scripts/05_build_technology_hardware_market_features.py"), asof_args),
        ...
        Step("07_sync_sec_fundamentals", "stage_4", "Sync SEC submissions/companyfacts", py_script("technology/technology_hardware/scripts/07_sync_technology_hardware_sec_fundamentals.py"), [*sec_args, "--allow-partial"], network=True),
        Step("07b_recover_6k_financials", "stage_4", "Recover cached foreign-filer 6-K financial facts", py_script("technology/scripts/07b_recover_technology_6k_financials.py"), ["--family", "technology_hardware", *asof_args]),
        ...
        Step("06a_validate_scoring_contract", "stage_6a", "Validate technology-hardware scoring feature contract", py_script("technology/technology_hardware/scripts/06a_validate_technology_hardware_scoring_features.py"), asof_args),
        Step("06b_validate_overlay_closure", "stage_6b", "Validate deliberate neutral/no-overlay closure", py_script("technology/technology_hardware/scripts/06b_validate_technology_hardware_overlay_closure.py"), asof_args),
        Step("10_build_stage7_scores", "stage_7", "Build production calibrated scores", py_script("technology/technology_hardware/scripts/10_build_technology_hardware_calibrated_scores.py"), asof_args),
        Step("10_validate_stage7_scores", "stage_7", "Validate production calibrated scores", py_script("technology/technology_hardware/scripts/10_validate_technology_hardware_calibrated_scores.py"), asof_args),
        Step("10_build_stage7_reference", "stage_7", "Build Stage 7 reference/challenger scores", py_script("technology/technology_hardware/scripts/10_build_technology_hardware_stage7_challenger_scores.py"), asof_args),
        Step("10_validate_stage7_reference", "stage_7", "Validate Stage 7 reference/challenger scores", py_script("technology/technology_hardware/scripts/10_validate_technology_hardware_stage7_challenger_scores.py"), asof_args),
        Step("07_signal_diagnostics", "research", "Run signal IC diagnostics", py_script("technology/technology_hardware/scripts/07_run_technology_hardware_signal_diagnostics.py"), research=True),
        Step("07_validate_signal_diagnostics", "research", "Validate signal diagnostics", py_script("technology/technology_hardware/scripts/07_validate_technology_hardware_signal_diagnostics.py"), pass_db=False, research=True),
        Step("08_run_optuna", "stage_8", "Run constrained Optuna calibration", py_script("technology/technology_hardware/scripts/08_run_technology_hardware_optuna_calibration.py"), optuna=True),
        Step("08_validate_optuna", "stage_8", "Validate Optuna calibration output", py_script("technology/technology_hardware/scripts/08_validate_technology_hardware_optuna_calibration.py"), optuna=True),
        Step("08c_walk_forward_calibration", "stage_8", "Run walk-forward calibration research", py_script("technology/technology_hardware/scripts/08c_run_technology_hardware_walk_forward_calibration.py"), optuna=True),
        ...
        Step("10c_financial_lineage_shadow", "stage_10_lineage", "Build blocking production financial-lineage sidecar", py_script("technology/scripts/10c_build_technology_financial_lineage_shadow.py"), ["--family", "technology_hardware", "--policy-context", "production", *asof_args]),
...
def main() -> int:
    ...
    output_dir = args.output_dir.expanduser().resolve() if args.output_dir else resolve_path(
        cfg_get(config, f"{CONFIG_KEY}.output_dir", "../output/technology_reports/technology_hardware/orchestration"),
        base_dir=base_dir,
    )
    ...
    summary = {
        ...
        "manifest_json": str(output_dir / "technology_hardware_refresh_manifest.json"),
        "manifest_csv": str(output_dir / "technology_hardware_refresh_steps.csv"),
        "steps": rows,
    }
    manifest_json = output_dir / "technology_hardware_refresh_manifest.json"
    manifest_csv = output_dir / "technology_hardware_refresh_steps.csv"
