#!/usr/bin/env python3
"""Load point-in-time technology-hardware research membership rows."""
...
LOGGER = logging.getLogger("load_technology_hardware_historical_membership")
DEFAULT_CONFIG = PACKAGE_ROOT / "config.yaml"
DEFAULT_MEMBERSHIP_CSV = PACKAGE_ROOT / "technology_hardware" / "data" / "technology_hardware_historical_membership.csv"
MODEL_FAMILY = "technology_hardware"
SOURCE_ID = "technology_hardware_historical_membership_seed"
RUN_TYPE = "load_technology_hardware_historical_membership"
POSITIONING_AMBIGUOUS_EXCHANGE_TICKERS = {"DELL", "PAR"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load technology-hardware historical/delisted PIT membership.")
...
def read_membership_csv(path: Path) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        row["price_source_symbol"] = str(row.get("price_source_symbol") or "").strip().upper()
...
def insert_identifier(
    ...
    *,
    ...
    conn.execute(
        ...
        """,
        (company_id, identifier_type, identifier_value, source_id_or_none(conn, source_id), confidence, now, now),
    )
...
def load_current_pit_rows(conn: Any, *, source_id: str, price_source: str, optimization_start: str) -> int:
    rows = conn.execute(
        """
...
def delete_existing_current_memberships(conn: Any) -> int:
    """Keep one current PIT interval per active hardware ticker after this loader runs."""
...
def upsert_historical_company(conn: Any, row: dict[str, str], *, source_id: str) -> int:
    now = utc_now()
    ticker = normalize_ticker(row["ticker"])
    ...
    cohort_id = str(row.get("calibration_cohort_id") or "tech_hw_historical")
    cohort_name = str(row.get("calibration_cohort") or "Historical technology hardware membership")
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, 'Technology', 'Technology Hardware', 'Technology Hardware',
                ?, ?, 'historical', 0, 'historical_membership_seed', ?, ?)
        ON CONFLICT(ticker) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        (ticker, cik, company_name, country, currency, now, now),
    )
    ...
    if exchange_ticker and exchange_ticker != ticker and exchange_ticker not in POSITIONING_AMBIGUOUS_EXCHANGE_TICKERS:
        insert_identifier(
            conn,
    ...
    elif exchange_ticker and exchange_ticker != ticker:
        insert_identifier(
            conn,
            company_id=company_id,
            identifier_type="HISTORICAL_EXCHANGE_TICKER",
            identifier_value=f"{ticker}:{exchange_ticker}",
            source_id=source_id,
            confidence=0.90,
        )
    ...
    if price_source_symbol:
        insert_identifier(
            ...
            identifier_value=price_source_symbol,
            source_id=source_id,
            confidence=0.95,
        )
    ...
    conn.execute(
        ...
        VALUES (?, ?, ?, 'Technology', ?, ?, ?, ?, 'historical_backtest',
        ...
        (
            ...
            cohort_id,
            cohort_name,
            str(row.get("notes") or row.get("event_type") or "historical_technology_hardware")[:250],
...
def load_historical_rows(conn: Any, rows: list[dict[str, str]], *, source_id: str) -> int:
    count = 0
    for row in rows:
        ticker = normalize_ticker(row["ticker"])
        company_id = upsert_historical_company(conn, row, source_id=source_id)
        reason = "; ".join(
            part
            for part in (
                f"event_type={row.get('event_type')}",
                f"successor={row.get('successor_ticker')}",
                f"price_source_symbol={row.get('price_source_symbol')}",
...
def main() -> int:
    ...
    optimization_start = parse_date_text(
        cfg_get(config, "technology_hardware_universe.optimization_start_date", "2010-01-01"),
    ...
    with connect(db_path, timeout_sec=float(cfg_get(config, "runtime.sqlite_timeout_sec", 120.0))) as conn:
        ...
        try:
            with conn:
                ...
                pruned_current = delete_existing_current_memberships(conn)
                conn.execute(
                    f"""
                    DELETE FROM dim_identifier
                    WHERE identifier_type = 'EXCHANGE_TICKER'
                      AND identifier_value IN ({",".join("?" for _ in POSITIONING_AMBIGUOUS_EXCHANGE_TICKERS)})
                      AND company_id IN (
                          SELECT company_id
                    ...
                    """,
                    (*sorted(POSITIONING_AMBIGUOUS_EXCHANGE_TICKERS), MODEL_FAMILY),
                )
                ...
                historical_count = load_historical_rows(conn, rows, source_id=SOURCE_ID)
            finish_run(
                conn,
                run_id=run_id,
                status="success",
            ...
            LOGGER.info(
                "Loaded technology-hardware PIT membership: current=%d historical=%d pruned_current=%d db=%s",
                current_count,
                historical_count,
                pruned_current,
                db_path,
