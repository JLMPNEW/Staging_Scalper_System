...
from technology.technology_hardware.calibrated_scoring import SETTINGS as STAGE7_SETTINGS


LOGGER = logging.getLogger("technology_hardware_optuna")
...
def load_panel(config: dict[str, Any], base_dir: Path) -> tuple[list[dict[str, Any]], list[date], list[int]]:
    diagnostics_dir = resolve_path(
        cfg_get(config, f"{CONFIG_KEY}.signal_diagnostics_output_dir", cfg_get(config, "technology_hardware_signal_diagnostics.output_dir")),
        base_dir=base_dir,
    )
    panel_path = diagnostics_dir / "signal_panel.csv"
    if not panel_path.exists():
        raise RuntimeError(f"Missing Stage 8A panel. Run technology hardware signal diagnostics first: {panel_path}")
    horizons = [int(value) for value in cfg_get(config, "technology_hardware_signal_diagnostics.horizons_trading_days", [21, 63])]
    excluded = set(cfg_get(config, "technology_hardware_signal_diagnostics.excluded_subfeatures", []) or [])
...
def load_eval_kwargs(config: dict[str, Any]) -> dict[str, Any]:
    return {
        "neutral_score": float(cfg_get(config, "technology_hardware_calibrated_scoring.neutral_score", 50.0)),
        ...
        "min_cross_section": int(cfg_get(config, "technology_hardware_signal_diagnostics.min_cross_section", 30)),
...
def current_candidate_scores(config: dict[str, Any], db_path: Path, candidate: Candidate, neutral_score: float) -> list[dict[str, Any]]:
    ...
    stage7_source = str(cfg_get(config, f"{CONFIG_KEY}.stage7_source_id", "technology_hardware_calibrated_score_v1"))
    model_family = str(cfg_get(config, f"{CONFIG_KEY}.model_family", "technology_hardware"))
...
def run_technology_hardware_optuna_calibration() -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)sZ %(levelname)s %(name)s %(message)s", datefmt="%Y-%m-%dT%H:%M:%S")
    args = parse_args("Run Stage 8B constrained Optuna calibration for technology hardware.")
    ...
    study_name = f"hardware_stage8b_{datetime.now(timezone.utc):%Y%m%dT%H%M%SZ}"
    ...
    best_weights = {
        "source_id": str(cfg_get(config, f"{CONFIG_KEY}.source_id", "technology_hardware_stage8_optuna_calibration")),
        "model_family": str(cfg_get(config, f"{CONFIG_KEY}.model_family", "technology_hardware")),
...
def validate_technology_hardware_optuna_calibration() -> int:
    logging.basicConfig(level=logging.INFO, format="%(asctime)sZ %(levelname)s %(name)s %(message)s", datefmt="%Y-%m-%dT%H:%M:%S")
    args = parse_args("Validate Stage 8B constrained Optuna calibration outputs for technology hardware.")
    config_path = args.config.expanduser().resolve()
    config = load_yaml(config_path)
    base_dir = config_path.parent
    output_dir = args.output_dir.expanduser().resolve() if args.output_dir else resolve_path(cfg_get(config, f"{CONFIG_KEY}.output_dir"), base_dir=base_dir)
...
def run_technology_hardware_walk_forward_calibration() -> None:
    logging.basicConfig(level=logging.INFO, format="%(asctime)sZ %(levelname)s %(name)s %(message)s", datefmt="%Y-%m-%dT%H:%M:%S")
    try:
        import optuna
        ...
        pass
    args = parse_args("Run Stage 8C walk-forward refit validation for technology hardware.")
    config_path = args.config.expanduser().resolve()
    config = load_yaml(config_path)
    base_dir = config_path.parent
...
def validate_technology_hardware_walk_forward_calibration() -> int:
    logging.basicConfig(level=logging.INFO, format="%(asctime)sZ %(levelname)s %(name)s %(message)s", datefmt="%Y-%m-%dT%H:%M:%S")
    args = parse_args("Validate Stage 8C walk-forward calibration outputs for technology hardware.")
    config_path = args.config.expanduser().resolve()
    config = load_yaml(config_path)
    base_dir = config_path.parent
    base_output_dir = args.output_dir.expanduser().resolve() if args.output_dir else resolve_path(cfg_get(config, f"{CONFIG_KEY}.output_dir"), base_dir=base_dir)
