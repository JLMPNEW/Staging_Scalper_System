#!/usr/bin/env python3
"""Run the validated semiconductor refresh sequence.

The default path refreshes production inputs, rebuilds Stage 6/7 scores, publishes
reports/governance artifacts, and runs the final audit. Research diagnostics,
Optuna calibration, and one-time Norgate backfill are opt-in so routine refreshes
do not accidentally change the model-review surface.
"""
...
CONFIG_KEY = "semiconductor_refresh_pipeline"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the semiconductor production refresh pipeline.")
    ...
    parser.add_argument("--manual-wsts-xlsx", type=Path, default=None, help="Optional local WSTS workbook passed to the WSTS sync step.")
...
def build_steps(
    *,
    ...
    semiconductor_market_validate_args = [
        *semiconductor_market_args,
        "--policy",
        str(py_script("technology/semiconductors/data/semiconductor_universe_policy.yaml")),
    ]
    ...
    if manual_wsts_xlsx:
        wsts_args.extend(["--manual-xlsx", str(manual_wsts_xlsx.expanduser().resolve())])
    ...
    return [
        ...
        Step("01_load_universe", "stage_2", "Load current semiconductor universe", py_script("technology/semiconductors/scripts/01_load_semiconductor_universe.py")),
        Step("03_sync_prices", "stage_3", "Sync Yahoo adjusted prices and benchmarks", py_script("technology/scripts/03_sync_technology_yahoo_adjusted_prices.py"), yahoo_args, network=True),
        Step("01b_load_historical_membership", "stage_2", "Load PIT historical semiconductor membership", py_script("technology/semiconductors/scripts/01b_load_semiconductor_historical_membership.py")),
        Step("02_validate_universe", "stage_2", "Validate semiconductor universe", py_script("technology/semiconductors/scripts/02_validate_semiconductor_universe.py")),
        Step("04_audit_market_policy", "stage_3", "Audit market data source policy", py_script("technology/scripts/04_audit_technology_market_data_policy.py"), asof_args),
        ...
        Step("09_import_positioning", "stage_5", "Import positioning into technology.sqlite", py_script("technology/scripts/09_import_technology_positioning.py"), [*asof_args, *positioning_window_args]),
        Step("10_validate_positioning", "stage_5", "Validate SEC/positioning stage", py_script("technology/scripts/10_validate_technology_sec_positioning_stages.py")),
        Step("14_audit_form4_reconciliation", "stage_5", "Audit Form 4/direct ownership reconciliation", py_script("technology/scripts/14_audit_technology_form4_reconciliation.py"), asof_args),
        Step("06a_build_scoring_contract", "stage_6a", "Build semiconductor scoring feature contract", py_script("technology/semiconductors/scripts/06a_build_semiconductor_scoring_features.py"), asof_args),
        Step("06a_validate_scoring_contract", "stage_6a", "Validate semiconductor scoring feature contract", py_script("technology/semiconductors/scripts/06a_validate_semiconductor_scoring_features.py"), asof_args),
        Step("06b_sync_wsts", "stage_6b", "Sync WSTS billings overlay", py_script("technology/semiconductors/scripts/06b_sync_wsts_billings.py"), wsts_args, network=True),
        Step("06b_build_sector_cycle", "stage_6b", "Build WSTS sector-cycle features", py_script("technology/semiconductors/scripts/06b_build_sector_cycle_features.py"), asof_args),
        Step("06b_sync_big_tech_capex", "stage_6b", "Sync big-tech capex overlay", py_script("technology/semiconductors/scripts/06b_sync_big_tech_capex.py"), asof_args, network=True),
        Step("06b_build_big_tech_capex", "stage_6b", "Build big-tech capex features", py_script("technology/semiconductors/scripts/06b_build_big_tech_capex_features.py"), asof_args),
        Step("06b_apply_overlays", "stage_6b", "Apply semiconductor overlay scores", py_script("technology/semiconductors/scripts/06b_apply_semiconductor_overlay_scores.py"), asof_args),
        Step("06b_validate_overlays", "stage_6b", "Validate semiconductor overlays", py_script("technology/semiconductors/scripts/06b_validate_semiconductor_overlays.py"), asof_args),
        Step("10_build_stage7_scores", "stage_7", "Build calibrated Stage 7 scores", py_script("technology/semiconductors/scripts/10_build_semiconductor_calibrated_scores.py"), asof_args),
        Step("10_validate_stage7_scores", "stage_7", "Validate calibrated Stage 7 scores", py_script("technology/semiconductors/scripts/10_validate_semiconductor_calibrated_scores.py"), asof_args),
        Step("12_validate_research_hardening", "stage_8_hardening", "Validate research hardening prerequisites", py_script("technology/semiconductors/scripts/12_validate_semiconductor_research_hardening.py")),
        Step("07_signal_diagnostics", "research", "Run signal IC diagnostics", py_script("technology/semiconductors/scripts/07_run_semiconductor_signal_diagnostics.py"), research=True),
        Step("14_static_candidate_review", "research", "Review static Stage 7 candidate weights", py_script("technology/semiconductors/scripts/14_compare_semiconductor_stage7_static_candidates.py"), research=True),
        Step("09b_portfolio_backtest", "stage_9", "Run portfolio backtest reports", py_script("technology/semiconductors/scripts/09b_run_semiconductor_portfolio_backtest.py"), research=True),
...
def selected_steps(steps: list[Step], args: argparse.Namespace, config: dict[str, Any]) -> list[Step]:
    ...
    if include_optuna and not include_research:
        # Optuna validation is model-review research; include the cheaper
        # diagnostics/backtest context around it when Optuna is explicitly run.
...
def main() -> int:
    ...
    output_dir = args.output_dir.expanduser().resolve() if args.output_dir else resolve_path(
        cfg_get(config, f"{CONFIG_KEY}.output_dir", "../output/technology_reports/orchestration"),
        base_dir=base_dir,
    )
    ...
    summary = {
        ...
        "manifest_csv": str(output_dir / "semiconductor_refresh_steps.csv"),
        "steps": rows,
    }
    manifest_json = output_dir / "semiconductor_refresh_manifest.json"
