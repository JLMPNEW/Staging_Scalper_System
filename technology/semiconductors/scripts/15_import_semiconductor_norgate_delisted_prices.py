#!/usr/bin/env python3
"""One-time Norgate delisted price import for semiconductor historical members."""
...
LOGGER = logging.getLogger("import_semiconductor_norgate_delisted_prices")
DEFAULT_CONFIG = PACKAGE_ROOT / "config.yaml"
DEFAULT_OUTPUT = PROJECT_ROOT / "output" / "technology_reports" / "market_data" / "norgate_delisted_price_import.csv"
SOURCE_ID = "norgate_us_equities_total_return"
RUN_TYPE = "stage15_norgate_delisted_price_import"

# These are known historical-membership aliases where the local membership key
# differs from Norgate's source symbol. The DB rows keep the internal ticker key.
MANUAL_SOURCE_SYMBOLS = {
    "AQUA": "AQ-201909",  # Aquantia traded as AQ; AQUA is a bad stale alias in the seed.
    "NANO": "ONTO",  # Nanometrics lineage continued as Onto Innovation.
}
DUPLICATE_LINEAGE_INTERNAL_TICKERS = {"IIVI"}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import Norgate delisted semiconductor OHLCV rows.")
...
def choose_symbol(norgatedata: Any, ticker: str, end_date: str, delisted_symbols: set[str], current_symbols: set[str]) -> SymbolMatch:
    manual = MANUAL_SOURCE_SYMBOLS.get(ticker)
    if manual:
        if manual not in delisted_symbols and manual not in current_symbols:
            return SymbolMatch(None, "manual_source_symbol_missing")
        return SymbolMatch(manual, "manual_lineage_or_symbol_correction", security_name(norgatedata, manual))

    hits = [symbol for symbol in delisted_symbols if symbol == ticker or symbol.startswith(f"{ticker}-")]
    if not hits:
...
def cleanup_duplicate_lineage_prices(conn: sqlite3.Connection, source_id: str) -> int:
    if not DUPLICATE_LINEAGE_INTERNAL_TICKERS:
        return 0
    placeholders = ",".join("?" for _ in DUPLICATE_LINEAGE_INTERNAL_TICKERS)
    cur = conn.execute(
        f"DELETE FROM fact_price_ohlcv WHERE source_id = ? AND ticker IN ({placeholders})",
        (source_id, *sorted(DUPLICATE_LINEAGE_INTERNAL_TICKERS)),
    )
...
def ensure_source(conn: sqlite3.Connection, source_id: str, timestamp: str) -> None:
    conn.execute(
        ...
        (
            ...
            "fact_price_ohlcv",
            "stage8_calibration,stage9_walk_forward,stage15_survivorship_backfill",
            "technology",
            15,
            "active",
            "Historical/delisted semiconductor price backfill. Rows preserve internal historical ticker keys and record source_symbol in price_adjustment.",
            timestamp,
            timestamp,
        ),
    )
...
def main() -> int:
    ...
    membership_csv = resolve_path(cfg_get(config, "technology_universe.historical_membership_csv"), base_dir=PACKAGE_ROOT)
    ...
    try:
        if not args.dry_run:
            ...
            removed = cleanup_duplicate_lineage_prices(conn, args.source_id)
            if removed:
                LOGGER.info("Removed duplicate-lineage Norgate rows: tickers=%s rows=%d", sorted(DUPLICATE_LINEAGE_INTERNAL_TICKERS), removed)

        for member in members:
            lookup_ticker = member.ticker if member.ticker in MANUAL_SOURCE_SYMBOLS else member.exchange_ticker
            match = choose_symbol(norgatedata, lookup_ticker, member.end_date, delisted_symbols, current_symbols)
            status = "skipped"
            rows = 0
            first_bar = ""
            last_bar = ""
