...
def _choose_candidate(
    *,
    ...
    # A 252-session candidate consumes roughly one year of purge history. The
    # earliest expanding outer windows may therefore have no room for another
    # complete inner fold. In that case tune on the already-purged outer
    # training history. The outer test remains untouched and is still the sole
    # promotion evidence.
    evaluation_windows = (
        [fold["test_dates"] for fold in inner_folds] if inner_folds else [dates]
    )
    selection_method = "inner_walkforward" if inner_folds else "purged_outer_training"
    ...
    for candidate_index, candidate in enumerate(candidates, start=1):
        ...
        key = (objective, -int(candidate["max_holding_days"]))
        best_key = (
            best_objective,
            -int(best["max_holding_days"]) if best is not None else -10**9,
        )
...
def main() -> int:  # noqa: C901, PLR0912, PLR0915
    ...
    purge_signal_dates = int(
        math.ceil(max_holding_days / signal_every)
    ) + int(calibration_cfg.get("embargo_signal_dates", 2))
    if args.smoke:
    ...
    outer_folds = expanding_blocks(
        ...
        minimum_train_dates=int(calibration_cfg.get("minimum_outer_train_dates", 126)),
    ...
    try:
        with tempfile.TemporaryDirectory(dir=work_root) as temp_name:
            ...
            for outer in outer_folds:
                ...
                raw_test_dates = outer["test_dates"]
                outer_index = outer_folds.index(outer)
                next_start = (
    ...
    _mean, _se, active_t = mean_t_hac(
        list(daily_selection),
        max_lag=max(candidate["max_holding_days"] for candidate in candidates),
    )
