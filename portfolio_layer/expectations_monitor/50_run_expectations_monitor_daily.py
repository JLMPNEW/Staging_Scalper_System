...
SEMANTICS_SCRIPT = Path(__file__).with_name("41_validate_provider_estimate_semantics.py")
...
STATE_PIPELINE_SCRIPT = Path(__file__).with_name("59_run_expectations_state_pipeline.py")
STEP_FIELDS = [
    "step",
    "cycle",
    "status",
...
def _require_historical_event_skip(
    *,
    as_of: date,
    skip_event_cycle: bool,
    now_utc: datetime | None = None,
) -> None:
    """Prevent current-only outcome endpoints from being attributed to an older run."""
    current = now_utc or datetime.now(timezone.utc)
    if current.tzinfo is None:
        raise ValueError("now_utc must be timezone-aware")
    latest_session = latest_completed_session("XNYS", now_utc=current)
    if as_of > latest_session:
        raise ValueError(
            "Expectations-monitor as-of date exceeds the latest completed XNYS session: "
            f"as_of={as_of}; latest_completed={latest_session}"
        )
    if as_of < latest_session and not skip_event_cycle:
        raise ValueError(
            "Historical expectations-monitor runs require --skip-event-cycle; "
            "current provider outcomes cannot be backdated."
        )
...
def _outputs_valid(manifest_path: Path, manifest: dict[str, Any]) -> bool:
    ...
    if not isinstance(outputs, dict) or not outputs:
        return False
    return all(
        (manifest_path.parent / str(name)).is_file() and sha256_file(manifest_path.parent / str(name)) == str(expected)
    ...
    *,
    ...
    if acceptance in {"PASS", "PASS_WITH_WARNINGS"} and return_code == 0:
        return (
            "PASS",
            payload,
            f"acceptance={acceptance}; provider coverage gates passed; economic signals remain diagnostic",
        )
...
def _cycles_from_capture_steps(path: Path) -> list[str]:
    ...
    return sorted(cycle for cycle, providers in by_cycle.items() if providers == {"alpha_vantage", "fmp"})
...
def _prior_parent_valid(path: Path, *, plan_digest: str) -> bool:
    ...
    return all(
        ...
        and sha256_file(Path(str(child["manifest_path"]))) == str(child.get("manifest_sha256", ""))
...
def _publish_stable_manifest(
    *,
    ...
    payload = {
        ...
        "state_publication_authorized": bool(parent.get("state_publication_authorized", False)),
    ...
    if stable_path.is_file() and not force:
        ...
        if new_acceptance != "FAIL" and existing_acceptance != "FAIL":
            raise FileExistsError(
                f"Refusing to replace a different passing stable monitor manifest without --force: {stable_path}"
            )
    write_manifest(stable_path, payload)


def run_selftest() -> None:
    _require_historical_event_skip(
        as_of=date(2026, 7, 31),
        skip_event_cycle=False,
        now_utc=datetime(2026, 8, 3, 15, tzinfo=timezone.utc),
    )
    try:
        _require_historical_event_skip(
            as_of=date(2026, 8, 3),
            skip_event_cycle=True,
            now_utc=datetime(2026, 8, 3, 15, tzinfo=timezone.utc),
        )
    except ValueError as exc:
        assert "latest completed" in str(exc)
    else:
        raise AssertionError("Incomplete XNYS session was accepted")
    _require_historical_event_skip(
        as_of=date(2026, 8, 3),
        skip_event_cycle=False,
        now_utc=datetime(2026, 8, 4, 4, tzinfo=timezone.utc),
    )
    _require_historical_event_skip(
        as_of=date(2026, 7, 31),
        skip_event_cycle=True,
    ...
    try:
        ...
        raise AssertionError("Historical current-outcome event cycle was not rejected")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        method = "place" + "Order"
        (root / "unsafe.py").write_text(f"ib.{method}(contract, order)\n", encoding="utf-8")
        assert _broker_execution_surface_errors(root) == [f"unsafe.py:prohibited_ib_method:{method}"]
...
def main() -> int:
    ...
    allowed_providers = {str(value) for value in retention.get("allowed_providers", [])}
    configured_providers = {str(value) for value in dict(monitor_cfg.get("provider_capture", {})).get("providers", [])}
    ...
    try:
        ...
        conn.close()
    universe_dir = (
        paths.output_dir / "runs" / universe_as_of / str(monitor_cfg.get("output_subdir", "expectations_monitor"))
    )
    ...
    if not universe_ok:
        raise RuntimeError(f"Monitor universe {universe_as_of} is missing, stale, or hash-invalid; rerun script 39")
    ...
    plan_identity = {
        ...
        "provider_network_owner": ("independent_service" if independent_provider_owner else "daily_monitor"),
    ...
    stable_manifest_path = (
        paths.output_dir / "runs" / args.as_of.isoformat() / output_subdir / "daily_monitor_manifest.json"
    )
    stable_failed = False
    ...
    pending_requested = not args.skip_pending_orders and (
        args.capture_pending_orders or bool(pending_cfg.get("capture_enabled_by_default", True))
    )
    ...
    if pending_requested:
        ...
        if not valid:
            failed = True
        pending_hash = sha256_file(pending_manifest) if pending_manifest.is_file() else ""
        if pending_hash:
            children.append(
                {
                    "step": "pending_orders",
    ...
    if market_requested:
        ...
        validation_hash = sha256_file(validation_manifest) if validation_manifest.is_file() else ""
        steps.append(
            {
        ...
        market_status = "DRY_RUN" if args.dry_run else "PASS" if build_valid and validate_valid else "FAIL"
        market_detail = (
            "sealed adjusted OHLCV independently validated under Yahoo/IB/Tiingo policy"
            if market_status == "PASS"
            else validate_detail
            if build_valid
            else build_detail
        )
    ...
    steps.append(
        {
            ...
            "cycle": args.as_of.isoformat(),
            "status": ("DEFERRED" if skip_provider_capture else "PASS" if capture_valid else "FAIL"),
    ...
    if not args.dry_run:
        for index, cycle in enumerate(cycles, start=1):
            ...
            for step, script, child_output, child_name, extra in validations:
                ...
                child_hash = sha256_file(child_manifest) if child_manifest.is_file() else ""
                if child_hash:
                    children.append(
                        {
                            "step": step,
    ...
    if not args.skip_event_cycle:
        ...
        if event_manifest is None:
            ...
            event_manifest = event_output / "provider_event_cycle_manifest.json" if event_output is not None else None
        event_valid = (
            event_rc == 0
            and event_manifest is not None
            and _manifest_valid(
                event_manifest,
                accepted=({"DRY_RUN", "PASS", "PASS_NO_EVENTS"} if args.dry_run else {"PASS", "PASS_NO_EVENTS"}),
                identity=("universe_as_of", universe_as_of),
            )
        )
        event_hash = sha256_file(event_manifest) if event_manifest is not None and event_manifest.is_file() else ""
    ...
    if args.dry_run:
        ...
        diagnostics_status, diagnostics_payload, diagnostics_detail = _provider_diagnostics_result(
            diagnostics_manifest,
            return_code=diagnostics_rc,
            as_of_date=args.as_of.isoformat(),
        )
    diagnostics_hash = sha256_file(diagnostics_manifest) if diagnostics_manifest.is_file() else ""
    steps.append(
        {
    ...
    if run_state_chain and market_status == "PASS" and market_output is not None:
        ...
        state_payload = read_manifest(state_manifest_path) if state_valid and state_manifest_path.is_file() else {}
        state_status = (
            "DRY_RUN"
            if args.dry_run
            else "DEFERRED"
    ...
    print(f"cycles={len(cycles)}; state_publication_authorized={state_publication_authorized}; output={output_dir}")
