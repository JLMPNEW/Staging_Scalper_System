...
class Stage12Paths:
    def __init__(self, root: Path) -> None:
        ...
        self.sleeve_targets_csv = root / "machinery_production_sleeve_targets.csv"
        ...
        self.activation_state_json = root / "machinery_production_activation_state.json"
...
def _portfolio_family(
    ...
    matches = [
        item for item in families if isinstance(item, dict) and str(item.get("model_family") or "") == MODEL_FAMILY
    ]
    if len(matches) != 1:
        raise ValueError("portfolio_layer must configure exactly one machinery family")
...
def portfolio_activation_fingerprint(
    ...
    for family in families:
        if isinstance(family, dict) and str(family.get("model_family") or "") == MODEL_FAMILY:
...
def _score(
    ...
    for field in COMPONENT_FIELDS:
        ...
        if value is None:
            raise ValueError(f"{row.get('ticker')}: missing calibrated component {field}")
...
def production_preview_rows(
    ...
    *,
    ...
    for source in rows:
        row = {field: str(source.get(field) or "") for field in FINAL_RANK_FIELDS}
    ...
    if not sleeve_weights or any(weight <= 0 for weight in sleeve_weights.values()):
    ...
    for rank, (row, _) in enumerate(scored, start=1):
        ...
        rank_reason = str(row.get("rank_ready_reason") or "not_rank_ready")
        candidate_reason = (
            "ok"
            if selected
            else (
        ...
        row.update(
            {
                ...
                "portfolio_universe_eligible_flag": ("1" if universe_eligible else "0"),
                "portfolio_selection_policy": selection_spec.name,
                "portfolio_sleeve_selected_flag": ("1" if selected else "0"),
                "portfolio_sleeve_target_weight": (
                    f"{sleeve_weights[ticker]:.12f}".rstrip("0").rstrip(".") if selected else "0"
                ),
                "portfolio_candidate_gate": "1" if selected else "0",
                "portfolio_candidate_status": (
                    "eligible" if selected else ("not_selected" if universe_eligible else "not_eligible")
                ),
                "portfolio_candidate_reason": candidate_reason,
                "calibration_eligible_flag": ("1" if rank_eligible else "0"),
                "research_calibration_input_eligible_flag": ("1" if rank_eligible else "0"),
                "research_calibration_reason": ("ok" if rank_eligible else rank_reason),
                "calibration_sample_role": ("strict_oos" if rank_eligible else "excluded"),
                "stage11_calibration_panel_source": ("dashboard_rank_snapshot_current_universe_replay"),
                "stage11_calibration_input_eligible_flag": ("1" if rank_eligible else "0"),
                "stage11_calibration_input_reason": ("ok" if rank_eligible else rank_reason),
                "survivorship_corrected_panel_flag": "0",
                "oos_score_valid_flag": ("1" if rank_eligible else "0"),
                "oos_score_asof_date": (asof if rank_eligible else ""),
                "oos_invalid_reason": ("" if rank_eligible else rank_reason),
...
def _validate_preview_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        universe_eligible = str(row.get("portfolio_universe_eligible_flag") or "") == "1"
        universe_eligible_count += int(universe_eligible)
        selected = str(row.get("portfolio_sleeve_selected_flag") or "") == "1"
        ...
        if row.get("portfolio_selection_policy") != expected_policy:
            ...
            break
        try:
            target_weight = float(str(row.get("portfolio_sleeve_target_weight") or "0"))
        except ValueError:
        ...
        if (
            ...
            and (
                ...
                or row.get("portfolio_candidate_reason") != "outside_validated_top_quantile"
                or row.get("oos_score_valid_flag") != "1"
                or row.get("calibration_sample_role") != "strict_oos"
            )
        ):
        ...
        if not universe_eligible:
            core_excluded = row.get("portfolio_candidate_reason") == "development_stage_core_sleeve_excluded"
            if core_excluded:
                if (
                    row.get("oos_score_valid_flag") != "1"
                    or row.get("calibration_sample_role") != "strict_oos"
                    or row.get("research_calibration_input_eligible_flag") != "1"
                ):
                    issues.append(f"{ticker}: core-excluded research row lost OOS validity")
                    break
...
def build_stage12_lock(
    ...
    *,
    ...
    issues = [
        *[f"Stage 8: {item}" for item in stage8_validation.get("issues", [])],
        *[f"Stage 9: {item}" for item in stage9_validation.get("issues", [])],
    ]
    ...
    machinery_aum = float(cfg_get(config, "machinery_stage9.target_aum_usd", 0.0))
    portfolio_aum = float(cfg_get(portfolio_config, "transaction_costs.aum_usd", 0.0))
    if machinery_aum <= 0 or machinery_aum != portfolio_aum:
        raise ValueError(f"portfolio AUM mismatch machinery={machinery_aum} portfolio_layer={portfolio_aum}")
    ...
    stage8_acceptance = json.loads(stage8_acceptance_path.read_text(encoding="utf-8"))
    stage9_acceptance = json.loads(stage9_acceptance_path.read_text(encoding="utf-8"))
    selection_policy_raw = stage9_acceptance.get("production_selection_policy")
    ...
    if (
        selection_policy.get("version") != PRODUCTION_SELECTION_POLICY_VERSION
        or selection_policy.get("parity_status") != "PASS"
    ):
        raise ValueError("Stage 9 production selection policy is not sealed")
    recommended_variant = str(stage9_acceptance["recommended_variant_for_stage12"])
    ...
    if (
        selection_policy.get("portfolio_type") != selection_spec.portfolio_type
        or selection_policy.get("weighting") != selection_spec.weighting
        or float(selection_policy.get("quantile") or 0.0) != selection_spec.quantile
    ):
    ...
    fixed_equal_sleeves = {str(value) for value in optimizer_config.get("fixed_equal_weight_sleeves", [])}
    if selection_spec.weighting == "equal" and MODEL_FAMILY not in fixed_equal_sleeves:
        raise ValueError("portfolio_layer must preserve equal weights for machinery")
    ...
    proposed_portfolio_cap = float(cfg_get(config, f"{CONFIG_KEY}.proposed_portfolio_cap", 0.05))
    ...
    shadow_state = portfolio_family.get("required") is False and current_portfolio_cap == 0.0
    active_state = portfolio_family.get("required") is True and current_portfolio_cap == proposed_portfolio_cap
    previous_activation_state_sha256 = ""
    if shadow_state:
        activation_mode = ACTIVATION_MODE_INITIAL
        if active_state_path.exists():
            raise ValueError("Initial Stage 12 candidate conflicts with an existing machinery activation state")
    elif allow_active_upgrade and active_state:
        if not active_state_path.is_file():
            raise ValueError("Active-model replacement requires the current machinery activation state")
        previous_state = json.loads(active_state_path.read_text(encoding="utf-8"))
        if previous_state.get("acceptance") != "PASS" or previous_state.get("production_policy_status") != "ACTIVE":
            raise ValueError("Current machinery activation state is not active")
    ...
    weights = {str(key): float(value) for key, value in stage9_acceptance["recommended_weights"].items()}
    ...
    if not source_rank.exists() or not source_manifest.exists():
        raise FileNotFoundError(f"Stage 12 source dashboard is incomplete for {asof}")
    dashboard_manifest = json.loads(source_manifest.read_text(encoding="utf-8"))
    if dashboard_manifest.get("acceptance") != "PASS" or dashboard_manifest.get("rank_table_sha256") != file_sha256(
        source_rank
    ):
    ...
    preview_rows = production_preview_rows(
        ...
        asof=asof,
        lock_date=lock_date,
        score_model_version=str(cfg_get(config, f"{CONFIG_KEY}.score_model_version")),
        model_version=str(cfg_get(config, f"{CONFIG_KEY}.model_version")),
        scoring_contract_version=str(cfg_get(config, f"{CONFIG_KEY}.scoring_contract_version")),
        selection_spec=selection_spec,
        minimum_positions=minimum_positions,
        universe_policy=configured_universe_policy,
    )
    selected_count = sum(row["portfolio_sleeve_selected_flag"] == "1" for row in preview_rows)
    ...
    investable_count = sum(row.investable_eligible == 1 for row in adapter_result.rows)
    proposed_count = sum(row["portfolio_candidate_gate"] == "1" for row in preview_rows)
    if investable_count != proposed_count or proposed_count <= 0:
        raise ValueError("Stage 12 portfolio adapter did not preserve proposed candidates")
    selected_tickers = {row["ticker"] for row in preview_rows if row["portfolio_sleeve_selected_flag"] == "1"}
    adapter_tickers = {row.ticker for row in adapter_result.rows if row.investable_eligible == 1}
    if adapter_tickers != selected_tickers:
        raise ValueError("Stage 12 portfolio adapter changed selected sleeve membership")
    broad_eligible_count = sum(row["portfolio_universe_eligible_flag"] == "1" for row in preview_rows)
    lock = {
        ...
        "production_start_date": str(cfg_get(config, f"{CONFIG_KEY}.production_start_date")),
        "development_end_date": stage8_acceptance["development_end_date"],
        "lockbox_start_date": stage8_acceptance["sealed_start_date"],
        "recommended_model": stage9_acceptance["recommended_model_for_stage12"],
        "recommended_variant": stage9_acceptance["recommended_variant_for_stage12"],
        ...
        "portfolio_equal_weight_constraint_configured": (MODEL_FAMILY in fixed_equal_sleeves),
        "minimum_capacity_multiple": stage9_acceptance["minimum_capacity_multiple"],
        ...
        "stage8_run_manifest_sha256": file_sha256(stage8_paths(stage8_root).run_manifest_json),
        "stage8_run_manifest": str(stage8_paths(stage8_root).run_manifest_json),
        "stage9_run_manifest_sha256": file_sha256(stage9_paths(stage9_root).run_manifest_json),
        "stage9_run_manifest": str(stage9_paths(stage9_root).run_manifest_json),
        ...
        "machinery_portfolio_policy_sha256": (machinery_portfolio_policy_fingerprint(portfolio_config)),
        "portfolio_adapter_semantic_sha256": (industrial_adapter_semantic_sha256()),
        "portfolio_adapter_sha256": file_sha256(config_path.parents[2] / "portfolio_layer" / "scores" / "adapters.py"),
...
def validate_stage12_lock(
    *,
    ...
    broad_rows = [row for row in rows if row.get("portfolio_universe_eligible_flag") == "1"]
    if sleeve_rows != [{field: row.get(field, "") for field in SLEEVE_TARGET_FIELDS} for row in broad_rows]:
    ...
    result = {
        ...
        "broad_portfolio_universe_eligible_count": lock.get("broad_portfolio_universe_eligible_count"),
        "selected_sleeve_count": lock.get("selected_sleeve_count"),
        "proposed_portfolio_candidate_count": lock.get("proposed_portfolio_candidate_count"),
