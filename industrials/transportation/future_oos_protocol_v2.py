"""Transportation future evaluator with explicit equal-weight N/A scopes."""
...
from future_only_evidence.protocol import canonical_sha256, evaluate_future_evidence
...
def evaluate(
    *,
    ...
    """Evaluate ranked scopes and record eligibility-equal-weight scopes N/A.

    Eligibility-equal-weight groups have no cross-sectional selection spread,
    so claiming predictive pass/fail would be false.  They are shown explicitly
    as not applicable and excluded from the predictive group-pass denominator.
    """
    ...
    for path in capture_paths:
        ...
        for row in payload.get("signal_rows", []):
            if row.get("ranking_mode") == "eligibility_equal_weight":
                ...
                equal_weight.setdefault(key, set()).add(str(payload["capture_id"]))
    n_a_rows: list[dict[str, Any]] = []
    ...
    result["scope_verdicts"].extend(n_a_rows)
    applicable = [
        row
    ...
    result["group_scope_audit"] = {
        ...
        "not_applicable_equal_weight_verdict_count": len(n_a_rows),
        "not_applicable_excluded_from_group_pass_denominator": True,
    }
    result.pop("payload_sha256", None)
    result["schema_version"] = "transportation_future_only_evidence_evaluation_v2"
