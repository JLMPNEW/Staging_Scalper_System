...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import verified transportation Norgate PIT price history.")
...
def load_members(mapping_path: Path, membership_path: Path) -> list[ImportMember]:
    ...
    for row in read_csv_flexible(membership_path):
        if str(row.get("membership_status") or "").strip() not in {"delisted", "historical_delisted"}:
            continue
...
def add_issue(conn: Any, *, ticker: str, source_id: str, detail: str) -> None:
    ...
    conn.execute(
        ...
        VALUES (?, 'error', 'import_transportation_norgate_prices', ?, ?, ?, ?,
...
def main() -> int:
    ...
    import_config = family.get("norgate_delisted_import")
    if not isinstance(universe, dict):
        raise KeyError("model_families.transportation.universe must be a mapping")
    if not isinstance(import_config, dict):
        raise KeyError("model_families.transportation.norgate_delisted_import must be a mapping")
    ...
    source_id = str(import_config.get("source_id") or "norgate_us_equities_total_return")
    adjustment_mode = str(import_config.get("price_adjustment_mode") or "CAPITAL").strip().upper()
    purge_config = str(import_config.get("purge_existing_range", True)).strip().lower()
    ...
    with connection_context as conn:
        ...
        if conn is not None:
            ...
            run_id = start_run(conn, run_type="import_transportation_norgate_prices", input_path=mapping_path)
        try:
