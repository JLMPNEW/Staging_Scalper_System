...
from industrials.transportation.surface_semantic_review import REVIEW_POLICY_VERSION  # noqa: E402
...
REPLAY_FIELDS = (
    ...
    "denominator_concept", "source_document", "source_content_sha256", "evidence_key",
    "replay_status", "replay_reason", "review_policy_version", "reviewed_by", "reviewed_at",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay approved surface definitions without reparsing documents.")
...
def main() -> int:
    ...
    manifest_path = args.review_manifest.expanduser().resolve() if args.review_manifest else output_dir / "transportation_surface_semantic_review.json"
    ...
    for row in _csv(reviews_path):
        ...
        replay.append({
            ...
            "denominator_concept": row["denominator_concept"], "source_document": row["source_document"],
    ...
    replay_path = output_dir / "transportation_surface_semantic_replay.csv"
    accepted_path = output_dir / "transportation_surface_semantic_replay_accepted.csv"
    ...
    write_text_atomic(
        output_dir / "transportation_surface_semantic_replay.json",
