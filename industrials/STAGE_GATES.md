...
- `market_cap` is exported from PIT market-cap sources where available, then from PIT price times diluted shares, short-interest float-denominator proxies, or source-backed capacity overrides. `avg_dollar_volume_60d` must be populated; recent listings with fewer than 60 sessions use an available-history proxy with a clear `liquidity_capacity_reason`.
