...
The disclosure census reads only the exact bytes in the reconciled date seal and searches only documents selected into that snapshot. It cannot silently read a later mutable cache alias or all documents ever accumulated for an accession. The dedicated parser independently supplies the expected ingestion-config hash, recomputes the current issuer scope, validates exact lifecycle and manifest identities, and accepts only the sealed document inventory. The Consumer Defensive Stage 6B adapter now uses that boundary for measurement-only shadow extraction; historical inventory is complete, while production promotion remains disabled until the expanded reviewed golden corpus and later promotion gates exist.
...
The approved FDP-corrected rollout completed after a fresh chronological replay, transactionally consistent backups of both production databases, and a full production-copy rehearsal. Production Stages 2-4 now advance through `2026-08-14`. Stage 2 has 108 current and 11 reviewed historical securities with exactly 119 membership intervals; Stage 3 qualifies all 121 required series, selecting 107 Yahoo and 14 Norgate sources, and writes 108/108 full-quality current feature rows. Stage 4 seals 209,258 issuer-filing associations over 208,852 unique accessions, including 406 shared accessions. The association SHA-256 is `332edcf0e774cdfeebc4a44d1229f22875a3d4ca60158c4330dcdce277b40aa1`; the 1,287-file, 949,602,440-byte cache SHA-256 is `1567027272b517d5019d646eb74f777b829340f69435128acea9222f8cda445a`.

The deployed Stage 4 database contains 2,179,929 raw facts after inline-XBRL consolidation, 233,510 canonical facts, 49,879 FX rows including 52 quarantined, 119 financial-feature rows, 4,522 census-v3 summary rows, and 782 evidence rows. All 38 specialized metrics are loaded as measurement-only census outputs, all 119 reporting profiles are covered, and the live validator passes 40/40 checks with zero canonical FX gaps, lineage mismatches, stale lifecycle outputs, or foreign-key violations. Stage 4 production rollout is complete. Stage 6B now has a Consumer Defensive-owned numeric adapter, complete immutable historical filing/document inventory back to `2019-01-02`, and accepted shadow evidence; its next gates are reviewed recall/conflict adjudication and Stage 6C panel readiness.
...
The corrected rollout fixes the Fresh Del Monte public/provider identity split: the public security remains `FDP`, CIK `1047340`, while the reviewed Norgate identity is `DMC` with asset ID `132283`. The checked authoritative provider-symbol input prevents accidental assignment to DMC Global/`BOOM`. The rollout also adds five reviewed 13F/FINRA source identifiers, maps FINRA `BFB` to `BF-B`, and admits SJM's exact `Common Shares` title into the neutral Form 4 P/S derivation. The checksummed Stage 5 v2 migration separates issuer-level Section 16 source coverage from eligible purchase/sale transaction presence, and positioning definition v2 retains days-to-cover when no safe PIT float-share proxy exists.

Production Stage 5 is accepted at the latest fully supported PIT cutoff, `2026-08-11`, and passes 18/18 checks at the strict `100%` current threshold: 13F is 108/108, short-interest signal coverage is 108/108, and required positioning features are numeric for 108/108. Short-float percentage is safely available for 105/108; IMKTA, ODD, and STZ retain null ratios plus valid days-to-cover. Section 16 coverage is 95/95 applicable current domestic issuers; 13 current foreign private issuers are explicitly not applicable, so the raw 104-ticker Form 4 count is not the coverage denominator. The import contains 14,246 eligible ownership observations across 104 transaction tickers, 1,724 PIT 13F aggregates across 118 taxonomy tickers, and 12,567 FINRA rows across 115. Borrow remains optional and null.

The Stage 6A foundation audit passes and records `proceed_stage6a`; its earliest potential common-feature date is `2026-05-28`. The Consumer Defensive-owned Stage 6A implementation is complete. It freezes 17 common atomic components plus all 38 registered specialized component identities, performs deterministic cohort-percentile normalization with a full-universe fallback only when the configured peer/variance floor is met, preserves unavailable values as null, and keeps every component weight at zero. It does not import Technology scoring code and does not create final scores or ranks.

The isolated `2026-08-11` Stage 6A rehearsal passes 20/20 checks on a transaction-consistent Stage 5 copy: 108/108 PIT members, 108 scoring-input rows, and 5,940 component rows (1,836 common plus 4,104 specialized reservations). Ninety-four issuers are rank-ready and 14 have explicit financial-evidence review reasons, for `87.04%` readiness against the frozen `85%` gate. All 38 specialized metrics are present for every issuer as either `not_loaded` or `not_applicable`; their raw values, normalized values, scores, and weights remain null, null, null, and zero. The contract SHA-256 is `6ed3217de16d0f5d15fd2c8226ec696ac5e92744ea85d175d9a2c731e80a804d`.

The rollout retained pristine pre-change backups with SHA-256 `4a4eb865cea59eb9afdbaca5e236a4e2eee132445d6ad5f2567d9d009b70b822` for Consumer Defensive and `2a9f28516f7ed345419b799bed848133ce0c7a83f8d91dd54c6f6d4fe51bc35d` for the neutral positioning store. Production and backups pass SQLite integrity checks with zero foreign-key violations. The post-Stage-6A Consumer Defensive/shared-parser suite passes 423 tests with 6 platform-specific skips; Ruff and compile checks pass.

The isolated `2026-08-14` Stage 6B historical corpus is now complete under schema v5. The retained core snapshot still contains 3,061 governed annual/quarterly documents from `2019-01-02` with manifest SHA-256 `f909e150f351fa011ea86e522da7dd64902719b5cc8ab21018a9aa53cb4282cf`. The event hydrator additionally classified all 11,596 8-K/6-K filing indexes and sealed 20,001 governed documents: 11,596 primaries plus 8,405 results exhibits. Its 43,193 immutable index/document objects total 4,096,674,161 bytes and have manifest SHA-256 `d2647c2ec17a8aab901aa9d33bd77a183571d2791406ef5a1c73fb54e404d94e`; an exact cache-only replay reproduced the same count and hash with zero network access.

The final isolated extraction uses adapter v3.6 and extraction terms v6 against the unchanged 14,673-filing/23,078-document manifest (`dc0089866889a0a09e567fa4eb725539bdaa98c806f2e82017ba7b55672e9b6b`). Parser run 26 completed all 14,673 work items with zero work-item failures while linking 14,606 unchanged non-PDF work items and executing only 67 PDF-bearing items. The governed Tesseract 5.3.4 retry recovered 29 OCR evidence rows across six DEO/STZ metric pairs and reduced evidence-level PDF failures from 72 to 32; all OCR rows remain review-required and the six unresolved issuer/metric groups remain non-qualifying. Twenty-six non-OCR pairs remain explicitly ambiguous. Measurement run 23 remains the exact idempotent result: 3,820 observations (3,587 parser observations and 233 exact financial derivations), zero remaining ticker/metric/period/scope/unit conflicts, and qualified coverage of 478/1,079 all-taxonomy pairs (`44.30%`), 477/1,022 SEC-addressable pairs (`46.67%`), 457/971 current-live pairs (`47.06%`), and 456/916 current-live SEC-addressable pairs (`49.78%`). The unchanged coverage is expected because unreviewed OCR evidence cannot qualify. All ten Stage 6B validation checks pass, the exact policy SHA-256 is `c616a5a630ccf9720773fcfcfb01490c4acdf14622d82ee1d6a0a439821845d3`, and every specialized production weight remains zero.

The next extraction controls are implemented without changing the retained qualifying baseline. `13i_build_consumer_defensive_ambiguous_review.py` generated the current 26-pair/263-evidence-row review pack for 23 issuers at `outputs/stage6b_conflict_closeout_20260821/ambiguous_review_v37_ocr/consumer_defensive_ambiguous_review.csv`; its SHA-256 is `f3bc0fa1939673a2dd8c38ba8c53c2d3c373550676f6df33d1861e0f74eee269` and its pair-scope SHA-256 is `4c620e4052c5f233a844bf2741328fbcf8bfc3f77070f72b19b83c94de38bbcf`. It also writes a separate 29-row/six-pair OCR review file at `consumer_defensive_ocr_review.csv` with SHA-256 `daa92017df40bd792d5e7dc55503987e864e409e52d6a8c70123e228e6f0fa02`. Each row includes exact filing/accession/document identity, period, unit, scope, candidate value, source text, and provenance. `13j_replay_consumer_defensive_specialized_review.py` applies only enabled exact decisions to immutable base evidence, materializes a normal zero-provider parser run, and invokes the existing measurement and coverage build; it opens no documents and invokes neither Arelle, EdgarTools, nor OCR. OCR defaults remain resource-bounded to 12 pages, 144 DPI, eight seconds per page, 20 million pixels per page, and the 30-second document limit; run 26 used the explicitly governed 64-page/480-second retry bounds. The adapter binds both the Conda Tesseract executable and its English language data, records portable runtime/page/DPI provenance, and forces every plausible OCR candidate to `REVIEW_REQUIRED` at confidence no greater than `0.80`. Four documents above the existing byte/page caps remain blocked. OCR contract v2 changes work identity only for OCR-enabled PDF work, so every completed non-PDF item is reused.

The Stage 6B measurement build now deterministically rebuilds and verifies its exact Stage 4 canonical-financial prerequisite before deriving metrics. It fails closed on absent canonical facts, an incomplete 119-row feature snapshot, or any missing FX conversion. Fixed-charge coverage and lease-adjusted leverage use exact annual GAAP operating-income, depreciation, debt, cash, interest, lease-cost, and lease-liability facts. Revenue per unit and gross profit per case remain unavailable unless a filing supplies a compatible absolute physical-volume denominator; the code does not divide by a growth percentage or mix incompatible unit definitions.

`13d_build_consumer_defensive_historical_inventory.py` is the reusable planner, `13g_hydrate_consumer_defensive_historical_documents.py` seals governed annual/quarterly primaries, and `13h_hydrate_consumer_defensive_event_documents.py` validates each 8-K/6-K index and seals only its primary plus explicitly typed/described results exhibits. They coordinate with existing Stage 4 filing associations but do not rewrite Stage 4 facts, profiles, document bridges, reconciliations, or the SEC watermark. Subsequent refreshes can add newly accepted filings to new immutable Stage 6B snapshots; this is not a disposable one-time extractor. The older 100-cutoff `13f` executor remains a diagnostic/rebuild fallback, not the recommended historical metric path.
...
The latest measured post-baseline is schema v6, adapter v3.18, and extraction terms v8. Isolated parser run 42 completed all 14,673 filings and 23,078 documents with zero failed work; measurement run 37 passed 10/10 checks with zero conflicts. Breadth is 542/1,079 (50.23%) for all taxonomy pairs, 541/1,022 (52.94%) for SEC-addressable pairs, 505/971 (52.01%) for current-live pairs, and 504/916 (55.02%) for current-live SEC-addressable pairs. The run contains 8,816 accepted observations across 7,008 issuer-periods, including 461 multi-period issuer/metric pairs. Relative to v3.17, this is +30 all-taxonomy pairs, +30 SEC-addressable pairs, +17 current-live SEC-addressable pairs, and +4,438 observations. Remaining SEC-addressable gaps are 423 metric-targeted corpus/no-candidate pairs, 54 selective disclosures not confirmed, zero review-required pairs, and 4 parser-failure pairs.
...
Stage 0 registers candidate metrics and applicability. Stage 6B now extracts and reports technically usable measurements, but current evidence remains measurement-only. Diagnostics and Stage 8 decide whether any technically usable metric has enough history, breadth, and stability for nonzero scoring weight.
