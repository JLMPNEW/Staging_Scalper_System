...
Stage 4 schema evolution is an ordered, checksum-verified migration ledger through migration v9, with SEC ingestion-config v8 and issuer-scope contract v3. Migration units are immutable, run inside one transaction/savepoint, use bounded keyset backfills for large tables, verify backfill parity and foreign keys, and fail closed on a gap, future version, or checksum drift. Scope v3 binds normalized reporting currency in addition to ticker/company/CIK identity. Schema migration v8 quarantines older scope pointers non-destructively and invalidates trust when filing-company or reporting-currency inputs change; schema migration v9 adds the accession-keyed indexes required for bounded shared-filing reconciliation over raw facts, canonical facts, and disclosure-census evidence. SEC ingestion-config v8 treats Companyfacts' canonical base form as equivalent only to a recognized submissions amendment or transitional financial-form variant (for example, `10-K` with `10-K/A` or `10-KT`); cross-family conflicts still fail closed. Legacy projections cannot be treated as current reconciled evidence.
...
The current validator passes 38 of 39 checks. Its sole false check is `companyfacts_coverage_complete`: 113 of 119 reporting profiles are covered, and BTI, BUD, FMX, JBS, KOF, and UL require the explicit foreign-private-issuer/inline-XBRL fallback. The replay produced 230,913 canonical facts with zero missing canonical FX conversions and 119 feature rows: 11 complete, 85 partial, 3 missing, 9 stale, and 11 ineligible. The sealed census parsed 952 documents with zero parse failures and produced 4,522 summary rows, 781 evidence rows, and 38 metrics. The validator sees 956 hydrated document records in the database; 952 are selected into and parsed from the exact current seal.
...
Stage 4 is not closed because the Companyfacts coverage gate remains false and the separate census terminology review remains open. The six fallback names are coverage work, not evidence that canonicalization or the shared dedicated-parser kernel is defective, and they may not be waived implicitly. Stage 6B separately requires a complete historical filing/document inventory before Consumer Defensive shadow extraction; current-date seal readiness does not establish that historical inventory.
...
1. resolve or explicitly adjudicate the BTI, BUD, FMX, JBS, KOF, and UL Companyfacts/inline-XBRL fallback gap, and continue to disclose CLF until an in-window upstream series exists;
2. obtain final independent acceptance, then perform the reviewed production migration/rebuild and rerun the current validator;
3. close the stratified census hit/miss terminology review recorded in `CENSUS_TERMINOLOGY_AUDIT_2026-08-11.md`;
4. implement Stage 5 ownership, insider, and positioning history, then run the foundation-coverage audit and continuation review;
5. implement the stable Stage 6A common scoring-feature contract;
6. implement the Consumer Defensive Stage 6B metric adapter, complete historical filing/document inventory, shadow reconciliation, and golden-corpus validation; and
7. build and validate the definitive Stage 6C PIT historical feature panel before signal diagnostics.
