...
from consumer_defensive.core.norgate_membership import load_norgate_membership
...
from consumer_defensive.core.universe import load_policy, upsert_stage2_sources
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Load Consumer Defensive Norgate PIT index membership and listing history."
    )
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(
            "norgatedata is unavailable in this Python environment; run with the base Miniconda interpreter."
        ) from exc
    ...
    with connect(db_path, timeout_sec=timeout) as conn:
        ...
        run_id = start_run(
            conn,
            run_type="consumer_defensive_stage2_norgate_membership",
            input_path=args.policy,
        )
        try:
            ...
            finish_run(
                ...
                run_id=run_id,
                status="success",
                row_count=int(summary["daily_rows_written"]),
            ...
            raise
    print(json.dumps({"database": str(db_path), **summary}, indent=2, sort_keys=True))
