#!/usr/bin/env python3
"""Validate the Stage 4 SEC/FX/features/census checkpoint."""
...
from consumer_defensive.core.stage4 import bootstrap_stage4, validate_stage4
...
def main() -> int:
    ...
    with connect(db_path, timeout_sec=timeout) as conn:
        ...
        run_id = start_run(
            conn,
            run_type="consumer_defensive_stage4_validation",
            input_path=bundle.path,
        )
        try:
            result = validate_stage4(conn, bundle, as_of=args.as_of)
            finish_run(
                conn,
                run_id=run_id,
    ...
    payload = {"database": str(db_path), "as_of": args.as_of, **result}
    report_path = output_dir / "stage4_validation.json"
