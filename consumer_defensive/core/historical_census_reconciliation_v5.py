"""Strict, observed-only PIT census reconciliation.

V5 is additive because earlier reconciliation artifacts are immutable.  It
normalizes CSV 0/1 strings, distinguishes query errors from verified negatives,
and preserves a self-hashed reviewed override without treating current/final
taxonomy as point-in-time Consumer Defensive taxonomy.
"""
...
REVIEWED_PIT_OVERRIDE_SCHEMA_V1 = (
    'consumer_defensive_reviewed_pit_override_v1'
)
_OVERRIDE_FIELDS = {
...
def reviewed_pit_override_sha256(row: Mapping[str, Any]) -> str:
    payload = {
...
def _strict_overrides(
    ...
    for source in rows:
        ...
        if (
            ...
            or str(row['delisted_date']) != str(row['pit_overlap_end'])
        ...
        expected_hash = reviewed_pit_override_sha256(row)
        if str(row['record_sha256']).lower() != expected_hash:
...
def reconcile_historical_candidates_v5(
    conn: Any,
    rows: Sequence[Mapping[str, Any]],
    *,
    reviewed_pit_overrides: Sequence[Mapping[str, Any]] = (),
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Return the candidate/loaded union with honest PIT query states."""

    overrides = _strict_overrides(reviewed_pit_overrides)
    ...
    summary = {
        **summary,
        'status': 'HISTORICAL_CENSUS_RECONCILIATION_REVIEW_ONLY_V5',
