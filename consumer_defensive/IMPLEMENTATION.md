...
- `consumer_defensive/scripts/15_run_consumer_defensive_factor_validation.py`; and
- `consumer_defensive/scripts/15a_validate_consumer_defensive_factor_validation.py`.

Stage 6C owns scripts `14`/`14a`; shared-kernel factor validation owns `15`/`15a`.
...
Build the stable scoring-table contract from Consumer Defensive-owned market, financial, and positioning features. Technology code is not imported. Reserve cohort-specialized component rows without changing the table shape later.
...
Implementation status: complete. `core/stage6a_schema.py` provides a checksummed additive migration; `core/scoring_features.py` owns the 17-component common contract, 38-metric specialized reservation, deterministic normalization, lineage identities, build, and 20-check validator. Scripts `12` and `12a` publish the input/component matrix and validation artifacts.

The isolated `2026-08-11` rehearsal used a transaction-consistent Stage 5 copy and passed 20/20: 108 PIT inputs, 5,940 components, 94 rank-ready, 14 review-required, zero model outputs, and zero nonzero weights. The three partial issuers whose only financial measure has fewer than five comparable observations remain review-required; the implementation does not convert sparse values to a neutral score. Specialized numeric extraction remains Stage 6B work.
...
The shared-kernel intake controls remain the Stage 4/Stage 6B trust boundary. The Consumer Defensive sector adapter is now implemented and cannot parse a stale reconciliation, mutable cache alias, wrong issuer scope, mismatched direct-document list, or escaped path. The historical document inventory is complete; Consumer Defensive production promotion remains hard-disabled until the expanded reviewed golden corpus and later production gates exist.

Implemented code includes the sector-owned `consumer_defensive/adapters/dedicated_parser_adapter.py`, a Stage-6B-specific extraction-term registry that does not mutate the closed Stage 4 census vocabulary, checksummed additive Stage 6B schema v5, exact-seal manifest/inventory builder, immutable primary and event-document hydrators, reviewed recall audit, shadow runner, measurement-only promotion, financial derivations, coverage reports, overlay guard, and validator. Adapter v3.6 consumes the shared semantic HTML blocks while retaining Consumer Defensive metric logic, preserves section/table preambles and merged headers, supports bounded native PDF extraction with PyMuPDF fallback and optional bounded OCR, and uses terms v6 contextual aliases only when required KPI context is present and forbidden legal/risk/security context is absent. It normalizes decimal commas and directional signs; binds values to the correct sentence, bullet, row, column, section geography, product portfolio, and period; rejects guidance, targets, incentive-plan weights, ranges, dense unlabeled numeric sequences, flattened slide diagrams, malformed narrative tables, margin bridges, EPS/per-unit leakage, and generic volume/traffic/share language; preserves GAAP versus adjusted and named-segment/product/geography scope; applies amendment precedence and bounded rounding reconciliation; and never infers a report date later than filing acceptance. Definition conflicts are keyed by ticker, metric, period start/end, scope, and unit. Value-share and volume-share evidence retain distinct scopes. Exact currency-per-unit agricultural-processing and sales-per-square-foot measurements may qualify at their explicitly named operational segment scope, but broad segment revenue, volume, pricing, and store-count candidates remain review-only. A matched-scope organic-revenue/volume residual may create a clearly labeled derived price/unit observation only when direct evidence is absent. Exact financial derivations continue to include advertising intensity, gross-margin change, inventory turnover, net debt/EBITDA, net-store growth, excise-tax burden change, fixed-charge coverage, and lease-adjusted net leverage with raw-observation lineage. Every run rebuilds and verifies its canonical-financial prerequisite and fails closed on missing FX. Technology, Machinery, and other sector adapters are not imported or reused as Consumer Defensive metric logic.

The post-v3.6 extraction follow-up adds bounded OCR and an exact review bridge without changing existing non-OCR work identities. OCR defaults are capped at 12 pages, 144 DPI, eight seconds per page, 20 million pixels per page, and the existing 30-second/25 MB document limits; the sector runner accepts explicit bounded retry overrides. The adapter preflights and binds the Conda Tesseract executable plus English language data, stores engine/version/page/DPI/runtime provenance, and keeps plausible OCR candidates `REVIEW_REQUIRED` at confidence no greater than `0.80`. OCR contract v2 differs only for OCR-enabled PDF work. Run 26 used governed 64-page/480-second retry bounds, linked 14,606 non-PDF work items, executed 67 PDF-bearing items, recovered 29 review-required OCR rows across six DEO/STZ metric pairs, and reduced PDF parser-failure evidence from 72 to 32. Four byte/page-cap failures remain deliberately blocked. Script `13i` now emits both the 263-row/26-pair ambiguity pack and a separate 29-row/six-pair OCR pack; script `13j` provides immutable zero-provider policy replay, normal parser-run materialization, and the existing Stage 6B measurement/coverage rebuild. Base evidence never changes, unreviewed candidates never qualify, and production weights remain zero.

Specialized registry v2 adds `source_availability_class`. Thirty-five metrics remain in the SEC-addressable population and three are explicitly non-SEC. `stage6b_metric_coverage` preserves `registered_applicable` rows while adding `sec_addressable` rows; it never rewrites an absent measurement as zero. Schema v3 owns the immutable historical primary snapshot, v4 preserves independently hashed parser observations, and v5 adds a separate immutable 8-K/6-K snapshot plus mutually exclusive evidence-state coverage. `13h_hydrate_consumer_defensive_event_documents.py` validates each filing index, requires the exact primary, selects only EX-99 or results-context documents, seals index and document bytes, and binds the selection-policy hash. The multi-document source manifest records document role, SEC type, sequence, description, content type, and source kind. The parser independently revalidates the event snapshot and rebinds supplied paths to its exact CAS objects.

The planner and historical hydrator operate in coordination with the existing staging database but never reverse-mutate the canonical Stage 4 watermark, facts, profiles, document bridges, or reconciliation. Historical bytes live in the Stage 6B snapshot and are accepted by the shared parser only when they match that exact immutable seal and an active PIT filing association. A fresh canonical database is therefore no longer required for ordinary historical metric extraction; `13f` remains only a diagnostic/full-rebuild fallback. Adapter v3.6 retains the systemic resolution of all 51 previously blocked conflict groups and closes the final same-date geography/product-scope collisions, so no manual conflict verdict is required for the retained closeout. The remaining Stage 6B governance work is golden-corpus preservation/expansion and the unchanged Stage 6A rerun before any production promotion can be considered.

The retained core inventory contains 3,061 governed annual/quarterly filings. `13g_hydrate_consumer_defensive_historical_documents.py` sealed all 3,061 core documents (7,811,780,209 bytes) with manifest SHA-256 `f909e150f351fa011ea86e522da7dd64902719b5cc8ab21018a9aa53cb4282cf`; cache-only replay reproduced the same manifest without network access. `13h_hydrate_consumer_defensive_event_documents.py` then classified all 11,596 event candidates and sealed 20,001 documents as 43,193 immutable objects (4,096,674,161 bytes) with manifest SHA-256 `d2647c2ec17a8aab901aa9d33bd77a183571d2791406ef5a1c73fb54e404d94e`; its cache-only replay reproduced the exact count and hash.

The final isolated `2026-08-14` run uses adapter v3.6/terms v6. Its combined source manifest contains 14,673 filings and 23,078 documents with SHA-256 `dc0089866889a0a09e567fa4eb725539bdaa98c806f2e82017ba7b55672e9b6b`. Parser run 26 completed every work item with zero failures and preserved the retained qualifying result because its 29 OCR rows are review-only. Measurement run 23 therefore remains exact at 3,820 observations (3,587 parser and 233 derived), zero conflicts, and coverage of 478/1,079 all-taxonomy pairs (`44.30%`), 477/1,022 SEC-addressable pairs (`46.67%`), 457/971 current-live pairs (`47.06%`), and 456/916 current-live SEC-addressable pairs (`49.78%`). The prior adapter-v2.0 result of 511/1,079 is retained only as an overstated pre-closeout checkpoint; v3.6 removes false-positive and scope/period ambiguity while retaining only definition-complete observations. Validation passes 10/10 under policy SHA-256 `c616a5a630ccf9720773fcfcfb01490c4acdf14622d82ee1d6a0a439821845d3`. All measurements remain zero-weight and missing values remain null.
...
The latest measured Stage 6B result is schema v6, adapter v3.18, and extraction terms v8. Isolated parser run 42 completed 14,673 filings and 23,078 documents with zero failed work; measurement run 37 passed 10/10 checks with zero conflicts. Schema v6 reports ticker/metric breadth separately from historical depth: 542/1,079 (50.23%) all-taxonomy pairs, 541/1,022 (52.94%) SEC-addressable pairs, 505/971 (52.01%) current-live pairs, and 504/916 (55.02%) current-live SEC-addressable pairs, supported by 8,816 accepted observations across 7,008 issuer-periods and 461 multi-period issuer/metric pairs. Relative to v3.17, this is +30 all-taxonomy pairs, +30 SEC-addressable pairs, +17 current-live SEC-addressable pairs, and +4,438 observations. Missing pairs remain partitioned into metric-targeted no-candidate, incomplete work, targeted-but-unplanned, issuer-only corpus, selective-disclosure-not-confirmed, and absent-corpus states; the SEC-addressable residual is 423 targeted/no-candidate, 54 selective-disclosure-not-confirmed, zero review-required, and 4 parser-failure pairs.

The v3.18 aliases are punctuation-tolerant but context-gated, and prohibited legal, securities, risk, capacity-plan, and unquantified-level contexts remain rejected. Historical financial derivations are rebuilt at each revenue-filing acceptance date from canonical facts available by that date, beginning at the configured `historical_inventory_start`; current feature values are never carried backward. Script `13k` creates a seal-hash-bound PDF recovery manifest only for the four `parser_failure` SEC-addressable pairs, after which existing script `13a --enable-pdf-ocr` applies the unchanged review-only OCR contract. Because the maximum breadth gain is four pairs, OCR is optional cleanup rather than the next material coverage lever.

### Stage 6C - Final Historical Feature-Panel Readiness


Stage 6C is implemented in `stage6c_schema.py` and `stage6c_panel.py`, with scripts `14` and `14a`. It builds the exact monthly point-in-time specialized panel before signal diagnostics or shared factor validation. This is the definitive historical-readiness audit because it sees the final applicability masks, parser outcomes, definition versions, units, directions, and missing-data policies.
...
The retained Stage 6B historical replay is not the Stage 7 rehearsal database: it contains no Stage 5 `feature_positioning` rows. Rebuilding scoring-feature v2 there passes 19/20 structural checks but yields 0/108 rank-ready issuers and correctly fails the 85% floor. The next scoring rehearsal must start from a full-stack isolated clone with Stage 5 positioning loaded, rebuild Stage 6A v2, and then reapply the accepted Stage 6B overlay before any Stage 7 score or rank is constructed.


### Signal Diagnostics And Shared Factor Validation

Diagnostics run after the Stage 6 feature contract and before Stage 7 calibrated scoring. Scripts `15` and `15a` use the Consumer Defensive adapter to call the shared top-level `factor_validation` kernel, register sector/cohort families, publish evidence, and verify the immutable ledger. They do not import Technology scripts, write scoring tables, or create Portfolio Layer artifacts.
