...
SUMMARY_FIELDS = [
    ...
    "tier1_context_count",
    "tier1_missing_count",
    "tier1_high_confidence_allocation_count",
    "tier1_allocation_candidate_count",
    "tier1_research_watchlist_count",
    "tier1_low_priority_count",
    "tier1_rank_cap_veto_count",
]
...
def load_rows(conn: sqlite3.Connection, asof_date: str) -> list[dict[str, Any]]:
    rows = conn.execute(
        ...
        SELECT
            ...
            s.tier1_score_tier, s.tier1_allocation_eligible, s.tier1_research_watchlist,
            s.tier1_score_spread_to_allocation, s.tier1_score_spread_to_high_confidence,
            s.tier1_rank_quality_cap, s.tier1_rank_quality_cap_reasons,
            s.tier1_rank_quality_cap_vetoed, s.tier1_rank_quality_cap_veto_reasons,
            s.tier1_mature_defensive_score, s.tier1_expected_return_quality_score,
            s.tier1_value_trap_score, s.tier1_leverage_score, s.tier1_leverage_fragility_score,
            s.tier1_no_forward_guidance_flag, s.tier1_guidance_stale_flag,
            s.tier1_no_guidance_negative_growth_flag,
            s.tier1_production_policy_quality_penalty, s.tier1_production_policy_quality_bonus,
...
def build_summary(rows: list[dict[str, Any]], candidate_rows: list[dict[str, Any]], top_n: int, asof_date: str) -> dict[str, Any]:
    ...
    tier1_tiers = [str(row.get("tier1_score_tier") or "") for row in rows]
    return {
        "asof_date": asof_date,
