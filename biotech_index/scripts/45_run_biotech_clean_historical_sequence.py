...
def panel_qa(
    ...
    *,
    ...
    start_asof: str,
    end_asof: str,
    min_short_pct_coverage: float = 0.95,
    ...
    with closing(connect_readonly(db_path)) as conn:
        ...
        for asof in dates:
            ...
            if row["short_pct_available_rows"] < short_pct_min:
                failures.append("short_interest_pct_float_coverage")
        ...
        summary_rows.append(
            {
                ...
                "value": len(dates),
                "details": f"start={start_asof}; end={end_asof}",
            }
        )
        summary_rows.append(
            {
                "check": "row_count_and_short_pct_coverage",
                "status": "PASS" if all(row["status"] == "PASS" for row in detail_rows) else "FAIL",
                "value": sum(1 for row in detail_rows if row["status"] == "FAIL"),
                "details": f"expected_tickers={expected_count}; min_short_pct_coverage={min_short_pct_coverage:.2f}",
            }
        )
        summary_rows.append(
            {
...
def main() -> None:
    ...
    final_manifest = {
        ...
        "optuna_status": "not_run_waiting_for_clean_panel_ic_and_candidate_calibration_review",
    }
