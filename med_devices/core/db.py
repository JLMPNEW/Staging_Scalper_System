...
SAFE_COLUMN_TYPE_RE = re.compile(
    r"^(TEXT|INTEGER|REAL|BLOB|NUMERIC)(?:\s+NOT\s+NULL)?(?:\s+DEFAULT\s+(?:NULL|[-+]?\d+(?:\.\d+)?|'[^']*'))?$",
    re.IGNORECASE,
)
...
CREATE TABLE IF NOT EXISTS feature_fda_product_risk (
    ...
    fda_event_risk_product_family_adjusted_score REAL,
    fda_safety_product_family_adjusted_score REAL,
    fda_product_family_shadow_available_flag INTEGER DEFAULT 0,
    fda_product_family_shadow_oos_valid_flag INTEGER DEFAULT 0,
    fda_product_family_adjustment_applied_flag INTEGER DEFAULT 0,
    fda_product_family_exposure_available_count INTEGER DEFAULT 0,
    fda_product_family_exposure_waived_count INTEGER DEFAULT 0,
    fda_product_family_exposure_missing_count INTEGER DEFAULT 0,
    fda_product_family_shadow_status TEXT DEFAULT '',
    fda_product_family_shadow_reason TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS med_device_daily_scores (
    ...
    safe_core_model_version TEXT DEFAULT 'safe_core_v1_shadow',
    ...
    ic_tilted_composite_mode TEXT DEFAULT 'disabled',
    ic_tilted_component_ics_json TEXT DEFAULT '{}',
    cohort_percentile REAL,
    fundamental_quality_score REAL,
    fundamental_quality_component_weight REAL DEFAULT 0.0,
    ...
    durable_growth_signal_mode TEXT DEFAULT 'neutral_overlay',
    durable_growth_signal_direction TEXT DEFAULT 'neutral',
    durable_growth_signal_reliability REAL DEFAULT 0.0,
    durable_growth_score_source TEXT DEFAULT 'neutral',
    durable_growth_gate_mode TEXT DEFAULT 'overlay_only',
    durable_growth_policy_reason TEXT DEFAULT '',
    durable_growth_gate_excluded INTEGER DEFAULT 1,
    ...
    durable_growth_validation_status TEXT DEFAULT 'disabled',
    durable_growth_validation_reason TEXT DEFAULT '',
    durable_growth_production_state TEXT DEFAULT 'disabled',
    fda_product_score REAL,
    fda_product_score_legacy REAL DEFAULT 50.0,
    fda_alpha_score REAL DEFAULT 50.0,
    fda_safety_score REAL DEFAULT 50.0,
    fda_clearance_velocity_raw REAL,
    fda_clearance_velocity_score REAL DEFAULT 50.0,
    fda_clearance_acceleration_raw REAL,
    fda_clearance_acceleration_score REAL DEFAULT 50.0,
    fda_evidence_quality_score REAL DEFAULT 50.0,
    ...
    fda_score_source TEXT DEFAULT 'fda_product_score',
    fda_gate_mode TEXT DEFAULT 'hard_positive',
    ...
    reimbursement_component_weight REAL DEFAULT 0.0,
    reimbursement_status TEXT,
    direct_code_evidence INTEGER,
    payment_rate_evidence INTEGER,
    coverage_policy_evidence INTEGER,
    ...
    valuation_component_weight REAL DEFAULT 0.0,
    technical_entry_score REAL,
    technical_trend_quality_score REAL DEFAULT 50.0,
    technical_relative_strength_score REAL DEFAULT 50.0,
    technical_liquidity_score REAL DEFAULT 50.0,
    technical_volume_breakout_score REAL DEFAULT 50.0,
    technical_volatility_risk_score REAL DEFAULT 50.0,
    technical_setup_score REAL DEFAULT 50.0,
    technical_core_score REAL DEFAULT 50.0,
    technical_alpha_score REAL DEFAULT 50.0,
    technical_pullback_score REAL DEFAULT 50.0,
    ...
    technical_score_source TEXT DEFAULT 'legacy_setup',
    technical_entry_status_score REAL,
    technical_entry_status_score_source TEXT DEFAULT 'legacy_setup',
    borrow_availability_score REAL DEFAULT 50.0,
    borrow_fee_score REAL DEFAULT 50.0,
    borrow_squeeze_risk_score REAL DEFAULT 50.0,
    borrow_pressure_score REAL DEFAULT 50.0,
    ...
    sentiment_catalyst_component_weight REAL DEFAULT 0.0,
    value_trap_score REAL,
    ...
    entry_status TEXT,
    technical_gate_mode TEXT DEFAULT 'hard_positive',
...
CREATE INDEX IF NOT EXISTS idx_fact_fda_adverse_company_event_date ON fact_fda_adverse_event(company_id, event_date);
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    _ensure_table_optional_columns(
        ...
        {
            # identity / versioning — scoring_model_version nullable per SCHEMA; others have empty defaults
            "scoring_model_version": "TEXT",
            "score_model_version": "TEXT DEFAULT ''",
            "model_family": "TEXT DEFAULT ''",
            "model_version": "TEXT DEFAULT ''",
            ...
            "price_data_asof_date": "TEXT DEFAULT ''",
            "feature_data_asof_date": "TEXT DEFAULT ''",
            # recovery / equity disposition fields
            "recovery_type": "TEXT DEFAULT ''",
            "equity_recovery": "INTEGER DEFAULT 0",
            "drop_otc_tape": "INTEGER DEFAULT 0",
            # source as-of date fields
            "financial_data_asof_date": "TEXT DEFAULT ''",
            "short_interest_asof_date": "TEXT DEFAULT ''",
            "institutional_data_asof_date": "TEXT DEFAULT ''",
            "insider_data_asof_date": "TEXT DEFAULT ''",
            "borrow_data_asof_date": "TEXT DEFAULT ''",
            # forward catalyst fields (nearest_days and confidence nullable = no upcoming catalyst / unknown confidence)
            "forward_catalyst_event_date": "TEXT DEFAULT ''",
            "forward_catalyst_event_type": "TEXT DEFAULT ''",
            "forward_catalyst_nearest_days": "INTEGER DEFAULT NULL",
            "forward_catalyst_source": "TEXT DEFAULT ''",
            "forward_catalyst_confidence": "REAL DEFAULT NULL",
            "forward_catalyst_asof_date": "TEXT DEFAULT ''",
            # composite score outputs — nullable per SCHEMA (not yet computed)
            "composite_score": "REAL",
            "raw_composite_score": "REAL",
            "composite_percentile": "REAL",
            # calibration fields — defaults match SCHEMA sentinels
            "calibration_cohort": "TEXT",
            "calibration_status": "TEXT DEFAULT 'production_eligible'",
            "calibration_status_reason": "TEXT DEFAULT ''",
            "calibration_eligible_flag": "INTEGER DEFAULT 1",
            ...
            "single_product_risk_flag": "INTEGER DEFAULT 0",
            "binary_event_risk_flag": "INTEGER DEFAULT 0",
            # tier-1 safety gate — DEFAULT 1 = passed unless explicitly failed
            "tier1_safety_status": "TEXT DEFAULT 'pass'",
            "tier1_safety_reason": "TEXT DEFAULT ''",
            "passed_tier1_safety_gate": "INTEGER DEFAULT 1",
            ...
            "legacy_all_gates_gate": "INTEGER DEFAULT 0",
            "legacy_gate_misses": "TEXT DEFAULT ''",
            # IC-tilted composite fields
            "ic_tilted_composite_score": "REAL DEFAULT 0.0",
            "ic_tilted_composite_delta": "REAL DEFAULT 0.0",
            "ic_tilted_composite_mode": "TEXT DEFAULT 'disabled'",
            "ic_tilted_component_ics_json": "TEXT DEFAULT '{}'",
            # cohort percentile — nullable (not all cohorts have enough members)
            "cohort_percentile": "REAL",
            # component scores — nullable per SCHEMA (score not computed for this row)
            "fundamental_quality_score": "REAL",
            "fundamental_quality_component_weight": "REAL DEFAULT 0.0",
            "durable_growth_score": "REAL",
            "durable_growth_score_legacy": "REAL DEFAULT 50.0",
            ...
            "data_completeness_score": "REAL",
            "live_component_count": "INTEGER",
            # delta / change tracking — nullable (only populated on incremental runs)
            "composite_score_delta": "REAL",
            "rank_delta": "INTEGER",
            "classification_change": "TEXT",
            "rank": "INTEGER",
            ...
            "technical_entry_status_score": "REAL",
            "technical_entry_status_score_source": "TEXT DEFAULT 'legacy_setup'",
            # positioning scores — DEFAULT 50.0 = neutral (no-data sentinel)
            "borrow_availability_score": "REAL DEFAULT 50.0",
            "borrow_fee_score": "REAL DEFAULT 50.0",
            "borrow_squeeze_risk_score": "REAL DEFAULT 50.0",
            "borrow_pressure_score": "REAL DEFAULT 50.0",
            ...
            "insider_activity_score": "REAL DEFAULT 50.0",
            "insider_data_quality_score": "REAL DEFAULT 0.0",
            # pullback candidate fields
            "pullback_candidate_tag": "INTEGER DEFAULT 0",
            "pullback_candidate_reason": "TEXT DEFAULT ''",
            "pullback_candidate_template_id": "TEXT DEFAULT ''",
            # gate / classification outputs — nullable (computed at score time)
            "gate_status": "TEXT",
            "review_reason": "TEXT",
            "failed_gates": "TEXT",
            "classification_reason": "TEXT",
            "fda_review_state": "TEXT",
            # market data fields — nullable (may be missing for delisted names)
            "market_cap": "REAL",
            "current_shares_outstanding": "REAL",
            "diluted_weighted_average_shares": "REAL",
            "basic_weighted_average_shares": "REAL",
            ...
            "min_position_size_feasible": "REAL",
            "max_position_size_feasible": "REAL",
            # gate pass/fail flags — nullable (not evaluated for non-candidate rows)
            "passed_raw_score_gate": "INTEGER",
            "passed_fundamental_gate": "INTEGER",
            "passed_growth_gate": "INTEGER",
            "passed_fda_gate": "INTEGER",
            ...
            "passed_valuation_gate": "INTEGER",
            "passed_technical_gate": "INTEGER",
            # DEFAULT 1 = veto not triggered; matches SCHEMA sentinel
            "passed_technical_breakdown_veto": "INTEGER DEFAULT 1",
            "passed_value_trap_gate": "INTEGER",
            "passed_data_quality_gate": "INTEGER",
            "passed_liquidity_gate": "INTEGER",
            "passed_fda_manual_review_gate": "INTEGER",
            "final_investability_gate": "INTEGER",
            # hard red flag — matches SCHEMA NOT NULL DEFAULT 0
            "hard_red_flag": "INTEGER NOT NULL DEFAULT 0",
            "hard_red_flag_reasons": "TEXT",
            "top_positive_drivers_json": "TEXT",
            "top_negative_drivers_json": "TEXT",
            # reimbursement detail fields — nullable per SCHEMA
            "reimbursement_status": "TEXT",
            "direct_code_evidence": "INTEGER",
            "payment_rate_evidence": "INTEGER",
            "coverage_policy_evidence": "INTEGER",
