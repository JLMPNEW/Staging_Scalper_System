...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test a technical-neutral gate for one med-device calibration cohort.")
...
def passes_gates(
    ...
    *,
    ...
    if fda_review_policy == "clean_or_cleared_only" and fda_state not in {"", "clean", "cleared", "low_materiality"}:
        return False
    if fda_review_policy == "exclude_manual_hard_red" and (fda_state in MANUAL_FDA_STATES or int_flag(row.get("hard_red_flag"))):
        return False
...
def evaluate(
    ...
    *,
    ...
    item: dict[str, Any] = {
        ...
        "min_avg_dollar_volume_60d": min_avg_dollar_volume_60d,
        "data_completeness_min": data_completeness_min,
        "entry_status_policy": "ignore_entry_status",
        "fda_review_policy": fda_review_policy,
        "reimbursement_policy": reimbursement_policy,
...
def main() -> None:
    ...
    cohort = args.cohort.strip() or str(cfg_get(config, "calibration.technical_neutral_gate_test.default_cohort", ""))
    if not cohort:
        raise ValueError("Provide --cohort or calibration.technical_neutral_gate_test.default_cohort")
    input_csv = (
    ...
    output_csv = path_from_config(base_dir, config, "calibration.technical_neutral_gate_test.output_csv", args.output_csv)
    recommendation_csv = path_from_config(
        base_dir,
        config,
        "calibration.technical_neutral_gate_test.recommendation_csv",
        args.recommendation_csv,
    )
    comparison_csv = path_from_config(base_dir, config, "calibration.technical_neutral_gate_test.comparison_csv", args.comparison_csv)
    ...
    section = "calibration.technical_neutral_gate_test"

    all_results: list[dict[str, Any]] = []
    for raw_min, pct_min, trap_max, liquidity_min, data_min, fda_policy, reimbursement_policy in itertools.product(
        parse_float_list(cfg_get(config, f"{section}.raw_score_min"), "45,50,52.5,55,57.5,60"),
        parse_float_list(cfg_get(config, f"{section}.cohort_percentile_min"), "0,25,40,50,60,70"),
        parse_float_list(cfg_get(config, f"{section}.value_trap_max"), "20,30,40,60"),
        parse_float_list(cfg_get(config, f"{section}.min_avg_dollar_volume_60d"), "0,1000000,2000000,5000000"),
        parse_float_list(cfg_get(config, f"{section}.data_completeness_min"), "85,90,100"),
        parse_str_list(cfg_get(config, f"{section}.fda_review_policy"), "exclude_manual_hard_red,clean_or_cleared_only"),
        parse_str_list(
            cfg_get(config, f"{section}.reimbursement_policy"),
    ...
    write_csv(
        ...
        [
            ...
            comparison_row("technical_neutral_best_gate", selected_rows, horizons=horizons),
        ],
        comparison_fields(horizons),
    )
    print(f"technical_neutral_gate_grid_csv={output_csv} rows={len(all_results)}")
    print(f"technical_neutral_recommendation_csv={recommendation_csv} pass_fail={best['pass_fail']}")
    print(f"technical_neutral_comparison_csv={comparison_csv}")
